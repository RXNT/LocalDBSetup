...
Description			:	This procedure is used to insert First Name Last Name Gender Matching
Last Modified By	:
Last Modifed Date	:
=======================================================================================
*/
CREATE PROCEDURE [rpt].[usp_InsertDeduplication_FnLnGenMatching]
(
	@CompanyId BIGINT,
...
BEGIN
	...
	WHERE	PST.Code = 'FLGNR'
	

	-- First Transition table	
	Select		dg.dc_id,  ISNULL(pat.pa_first, '')  + '~~' + ISNULL(pat.pa_last, '') + '~~' +
				ISNULL(pat.pa_sex, '') As DuplicationText
	INTO		#TempPatients
	...
	Group By	dg.dc_id, 
				ISNULL(pat.pa_first, '')  + '~~' + ISNULL(pat.pa_last, '') + '~~' +  ISNULL(pat.pa_sex, '')
	Having		COUNT(*) > 1
	...
	FROM		dbo.patients pat WITH (NOLOCK)
				...
				INNER JOIN rpt.DoctorCompanyDeduplicationTransition tmp WITH (NOLOCK) ON tmp.CompanyId = dg.dc_id 
					...
					AND ISNULL(pat.pa_first, '')  + '~~' + ISNULL(pat.pa_last, '') + '~~' + ISNULL(pat.pa_sex, '') = tmp.DuplicationText
	WHERE		dg.dc_id = @CompanyId
