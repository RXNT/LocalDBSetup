...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [erxd].[usp_GetProvidersForDisable] 
    @PartnerId  BIGINT,
    @version VARCHAR(10)
AS
BEGIN
	...
	where Not(spi_id Is Null) And dr_enabled = 0 And ss_enable = 1 And 
     D.dr_partner_participant = @PartnerId AND D.version= @version and prescribing_authority>2 AND update_date > getdate()-1
