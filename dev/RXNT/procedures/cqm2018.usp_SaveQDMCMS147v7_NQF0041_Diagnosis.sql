...
-- =============================================
CREATE PROCEDURE [cqm2018].[usp_SaveQDMCMS147v7_NQF0041_Diagnosis]
	@RequestId BIGINT
AS
BEGIN
	...
	INNER JOIN cqm2018.SysLookupCMS147v7_NQF0041 cqm WITH(NOLOCK) ON pad.icd9 = cqm.code 
	...
	pad.pa_id, pad.added_by_dr_id,cqm.CodeSystemId, cqm.Code, pad.date_added
	FROM patient_active_diagnosis pad WITH(NOLOCK)
	INNER JOIN cqm2018.SysLookupCMS147v7_NQF0041 cqm WITH(NOLOCK) ON pad.icd10 = cqm.code
	...
	pad.pa_id, pad.added_by_dr_id, cqm.CodeSystemId, cqm.Code, pad.date_added
	FROM patient_active_diagnosis pad WITH(NOLOCK)
	INNER JOIN cqm2018.SysLookupCMS147v7_NQF0041 cqm WITH(NOLOCK) ON pad.snomed_code = cqm.code
	LEFT JOIN cqm2018.PatientDiagnosisCodes pdc WITH(NOLOCK) on pad.pad = pdc.DiagnosisId AND pdc.EncounterId = EncounterId AND pdc.Code = cqm.Code
	WHERE pad.added_by_dr_id = @DoctorId
	AND pad.date_added between @StartDate AND @EndDate
	AND cqm.QDMCategoryId = 1  AND cqm.ValueSetId IN (48 ,49,59)
