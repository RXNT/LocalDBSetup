...
CREATE PROCEDURE [cqm2019].[usp_SavePop1NumeratorCMS69v7_NQF0421]
	@RequestId BIGINT  
AS
BEGIN
	...
	INNER JOIN cqm2019.PatientPhysicalExamCodes phec WITH(NOLOCK) ON IPP.PatientId = phec.PatientId
	INNER JOIN patient_vitals pv WITH(NOLOCK) ON pv.pa_id=pat.pa_id AND pv.pa_vt_id = phec.PhysicalExamId
	INNER JOIN cqm2019.PatientEncounterCodes pec WITH(NOLOCK) ON phec.EncounterId = pec.EncounterId
	...
	AND pec.PerformedFromDate BETWEEN @StartDate AND @EndDate
		...
		INNER JOIN cqm2019.PatientMedicationCodes pmc WITH(NOLOCK) ON pat.PatientId = pmc.PatientId AND PrescriptionId IS NOT NULL
		INNER JOIN cqm2019.[SysLookUpCMS69v7_NQF0421] cqm WITH(NOLOCK) on cqm.code = pmc.code
		INNER JOIN cqm2019.PatientEncounterCodes pec WITH(NOLOCK) ON pmc.EncounterId = pec.EncounterId
		...
		INNER JOIN cqm2019.[SysLookUpCMS69v7_NQF0421] cqm WITH(NOLOCK) on cqm.code = pic.code
		LEFT OUTER JOIN @temp3 t3 ON pat.PatientId=t3.PatientId
	...
	IF OBJECT_ID('tempdb..#usp_SavePop1NumeratorCMS69v7_NQF0421_1') IS NOT NULL DROP TABLE #usp_SavePop1NumeratorCMS69v7_NQF0421_1
	SELECT TOP(@MaxRowCount) num.CalcId
	INTO #usp_SavePop1NumeratorCMS69v7_NQF0421_1
	FROM 
	patients pat WITH(NOLOCK)
	INNER JOIN @temp3 t3 ON t3.PatientId=pat.pa_id 
	INNER JOIN cqm2019.[DoctorCQMCalcPop1CMS69v7_NQF0421] num WITH(NOLOCK) ON pat.pa_id=num.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	...
	SET Numerator = 1 
	FROM cqm2019.[DoctorCQMCalcPop1CMS69v7_NQF0421] num 
	INNER JOIN #usp_SavePop1NumeratorCMS69v7_NQF0421_1 temp WITH(NOLOCK) ON num.CalcId=temp.CalcId
	where ISNULL(num.Numerator,0)= 0 AND
	ISNULL(num.DenomExclusions,0)= 0 AND
	ISNULL(num.DenomExceptions,0)= 0 
	DROP TABLE #usp_SavePop1NumeratorCMS69v7_NQF0421_1
	...
	SELECT DISTINCT pat.PatientId FROM @temp2 pat
		...
		AND DATEDIFF(MONTH, pic.PerformedFromDate, pat.PerformedFromDate) <=12
	IF OBJECT_ID('tempdb..#usp_SavePop1NumeratorCMS69v7_NQF0421_2') IS NOT NULL DROP TABLE #usp_SavePop1NumeratorCMS69v7_NQF0421_2
	
	SELECT TOP(@MaxRowCount) num.CalcId
	INTO #usp_SavePop1NumeratorCMS69v7_NQF0421_2
	FROM cqm2019.[DoctorCQMCalcPop1CMS69v7_NQF0421] num WITH(NOLOCK) 
	...
	SET Numerator = 1 
	FROM cqm2019.[DoctorCQMCalcPop1CMS69v7_NQF0421] num WITH(NOLOCK) 
	INNER JOIN #usp_SavePop1NumeratorCMS69v7_NQF0421_2 temp WITH(NOLOCK) ON temp.CalcId=num.CalcId
	
	DROP TABLE #usp_SavePop1NumeratorCMS69v7_NQF0421_2
	SET @tempRowCount = @@ROWCOUNT;
	IF @tempRowCount > @AffectedRows
	BEGIN 
		SET @AffectedRows = @tempRowCount;
