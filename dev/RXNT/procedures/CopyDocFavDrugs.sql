...
-- =============================================
CREATE PROCEDURE [dbo].[CopyDocFavDrugs] 
...
BEGIN
    ...
    BEGIN
		...
		select @dr_id,drug_id from doc_fav_drugs where dr_id=@SOURCE_DR_ID and drug_id not in (select		drug_id from doc_fav_drugs where dr_id=@dr_id)
		
		insert into doc_fav_scripts (dr_id,ddid, use_generic , dosage,numb_refills,duration_amount,duration_unit,			comments,prn,prn_description,days_supply,compound)
		select @dr_id,A.ddid,A.use_generic,A.dosage,A.numb_refills,A.duration_amount,A.duration_unit,A.comments,A.prn		,A.prn_description,A.days_supply,A.compound from
