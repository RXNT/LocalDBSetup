...
CREATE PROCEDURE [cqm2018].[usp_GetDoctorQRDAIII_CMS155v6_NQF0024_POP2]  
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
	 ...
	 BEGIN
		 ...
		 SUM(CASE WHEN N.NumerExclusions = 1 THEN 1 ELSE 0 END) as NumerExclusions,
		 SUM(CASE WHEN N.DenomExceptions = 1 THEN 1 ELSE 0 END) as DenomExceptions
		 FROM  [cqm2018].[DoctorCQMCalcPop2CMS155v6_NQF0024] N  WITH(NOLOCK)
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop2CMS155v6_NQF0024] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop2CMS155v6_NQF0024] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
