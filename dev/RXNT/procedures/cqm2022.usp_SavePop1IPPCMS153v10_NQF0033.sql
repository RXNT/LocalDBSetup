...
-- =============================================
CREATE   PROCEDURE [cqm2022].[usp_SavePop1IPPCMS153v10_NQF0033]
	@RequestId BIGINT  
AS
BEGIN
	...
	IF OBJECT_ID('tempdb..#tempCMS153v10_NQF0033') IS NOT NULL 
	BEGIN  
		DROP TABLE #tempCMS153v10_NQF0033
    END
	
	SELECT DISTINCT pat.pa_id INTO #tempCMS153v10_NQF0033
	...
	LEFT JOIN cqm2022.DoctorCQMCalcPop1CMS153v10_NQF0033 IPP WITH(NOLOCK) ON IPP.PatientId = pat.pa_id AND IPP.RequestId = @RequestId
	...
	SELECT DISTINCT TOP(@MaxRecords) i.pa_id, 1, 1, @DoctorId, @RequestId FROM (
		...
		INNER JOIN [cqm2022].[SysLookupCMS153v10_NQF0033] as cqm_pad with(NOLOCK) on pdc.Code = cqm_pad.Code
		...
		INNER JOIN cqm2022.SysLookupCMS153v10_NQF0033 cqm WITH(NOLOCK) ON pmc.Code = cqm.code AND pmc.MedicationId IS NOT NULL
	...
	LEFT JOIN cqm2022.DoctorCQMCalcPop1CMS153v10_NQF0033 IPP WITH(NOLOCK) ON IPP.PatientId = i.pa_id AND IPP.RequestId = @RequestId
	...
	SELECT DISTINCT TOP(@MaxRecords) i.pa_id, 1, 1, @DoctorId, @RequestId FROM (
		...
		INNER JOIN cqm2022.SysLookupCMS153v10_NQF0033 as cqm with(NOLOCK) on pltc.Code = cqm.Code
		...
		INNER JOIN patients pat WITH(NOLOCK) ON tmp.pa_id=pat.pa_id
		INNER JOIN cqm2022.PatientMedicationCodes pmc WITH(NOLOCK) ON pmc.PatientId = tmp.pa_id
		INNER JOIN cqm2022.SysLookupCMS153v10_NQF0033 cqm WITH(NOLOCK) ON pmc.Code = cqm.code AND pmc.PrescriptionId IS NOT NULL
		WHERE pmc.PerformedFromDate BETWEEN @StartDate AND @EndDate
		...
		INNER JOIN cqm2022.SysLookupCMS153v10_NQF0033 cqm WITH(NOLOCK) on cqm.Code = ppc_s.Code
		...
		INNER JOIN cqm2022.SysLookupCMS153v10_NQF0033 as cqm with(NOLOCK) on pdsc.Code = cqm.Code
