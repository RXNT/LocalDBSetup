...
-- =============================================
CREATE PROCEDURE [cqm2019].[usp_SavePop1DenomExclusionsCMS153v7_NQF0033]
	@RequestId BIGINT 
AS
BEGIN
	...
	FROM [cqm2019].[DoctorCQMCalcPop1CMS153v7_NQF0033] IPP WITH(NOLOCK)
	INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN [cqm2019].PatientLaboratoryTestCodes pltc WITH(NOLOCK) ON pltc.PatientId = IPP.PatientId
	...
	AND cqm.QDMCategoryId = 9 AND cqm.CodeSystemId=13 AND cqm.ValueSetId = 86
	AND pltc.DoctorId = @DoctorId AND pltc.PerformedFromDate BETWEEN @StartDate AND @EndDate
	AND EXISTS (
		SELECT 1 FROM cqm2019.PatientDiagnosticStudyCodes pdsc  WITH(NOLOCK)
		...
		AND cqm.QDMCategoryId = 15 AND cqm.CodeSystemId = 13 AND cqm.ValueSetId = 91
	...
	AND pltc.DoctorId = @DoctorId AND pltc.PerformedFromDate BETWEEN @StartDate AND @EndDate
	AND EXISTS (
		SELECT 1 FROM cqm2019.PatientMedicationCodes pmc 
		INNER JOIN patients pat1 WITH(NOLOCK) ON pmc.PatientId=pat1.pa_id
		INNER JOIN [cqm2019].[SysLookupCMS153v7_NQF0033] as cqm with(NOLOCK) on pmc.Code = cqm.Code
		WHERE pmc.PatientId = IPP.PatientId AND pmc.PerformedFromDate BETWEEN @StartDate AND @EndDate
		AND cqm.QDMCategoryId = 5 AND cqm.CodeSystemId = 7 AND cqm.ValueSetId = 81
