...
-- =============================================
CREATE PROCEDURE [dbo].[CQM_CMS127v2_NQF0043_POP1_IPP] 
...
BEGIN
	...
	BEGIN
		...
		where
        datediff(month,PAT.pa_dob,CONVERT(datetime,@fromdate,101)) >= 65*12 
	END
	ELSE IF @type = 2 --All
	BEGIN
		SELECT COUNT(distinct PAT.pa_id)SM 
			...
			(
				...
				where cqm.version=2 AND cqm.NQF_id='0043' 
	...
	BEGIN
        ...
        datediff(month,PAT.pa_dob,CONVERT(datetime,@fromdate,101)) >= 65*12 GROUP BY pa_race_type
	END
	...
	BEGIN
        ...
        AND PP.date_performed between @fromdate AND @todate
        group by pa_id having count(pa_id) > 0)PPC on PAT.pa_id=PPC.pa_id where
        datediff(month,PAT.pa_dob,CONVERT(datetime,@fromdate,101)) >= 65*12 GROUP BY pa_ethn_type
	END
	...
	BEGIN
        ...
        AND PP.date_performed between @fromdate AND @todate
        group by pa_id having count(pa_id) > 0)PPC on PAT.pa_id=PPC.pa_id where
        datediff(month,PAT.pa_dob,CONVERT(datetime,@fromdate,101)) >= 65*12 GROUP BY pa_sex
	...
	BEGIN
        ...
        where cqm_pyr.version=2 AND cqm_pyr.nqf_id='0043' AND datediff(month,PAT.pa_dob,CONVERT(datetime,@fromdate,101)) >= 65*12 GROUP BY medicare_type_code
