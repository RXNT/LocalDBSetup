...
-- =============================================
CREATE PROCEDURE [cqm2019].[usp_SavePop1NumeratorCMS138v7_NQF0028]
	@RequestId BIGINT 
AS
BEGIN
	...
	IF OBJECT_ID('tempdb..#usp_SavePop1NumeratorCMS138v7_NQF0028_1') IS NOT NULL DROP TABLE #usp_SavePop1NumeratorCMS138v7_NQF0028_1

	
	SELECT TOP(@MaxRows) IPP.CalcId
	INTO #usp_SavePop1NumeratorCMS138v7_NQF0028_1
	FROM [cqm2019].[DoctorCQMCalcPop1CMS138v7_NQF0028] IPP WITH(NOLOCK)
...
--	INNER JOIN [cqm2019].[SysLookupCMS138v7_NQF0028] cqm_r WITH(NOLOCK) on cqm_r.Code = prc.Code
	...
	AND prc.PerformedFromDate BETWEEN @measureStartDate AND @EndDate AND EXISTS(SELECT TOP 1* FROM  [cqm2019].[SysLookupCMS138v7_NQF0028] cqm_r WITH(NOLOCK) WHERE cqm_r.QDMCategoryId = 17 AND cqm_r.CodeSystemId = 13 AND cqm_r.ValueSetId = 167 AND cqm_r.Code = prc.Code)
	...
	SET IPP.Numerator = 1
	FROM [cqm2019].[DoctorCQMCalcPop1CMS138v7_NQF0028] IPP WITH(NOLOCK)
	INNER JOIN #usp_SavePop1NumeratorCMS138v7_NQF0028_1 T1 WITH(NOLOCK) ON T1.CalcId=IPP.CalcId
