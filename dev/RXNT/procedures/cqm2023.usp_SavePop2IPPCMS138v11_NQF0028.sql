...
-- =============================================
CREATE    PROCEDURE [cqm2023].[usp_SavePop2IPPCMS138v11_NQF0028]
	@RequestId BIGINT  
AS
BEGIN
	...
	IF OBJECT_ID('tempdb..#usp_SavePop2IPPCMS138v11_NQF0028_1') IS NOT NULL DROP TABLE #usp_SavePop2IPPCMS138v11_NQF0028_1
	IF OBJECT_ID('tempdb..#usp_SavePop2IPPCMS138v11_NQF0028_2') IS NOT NULL DROP TABLE #usp_SavePop2IPPCMS138v11_NQF0028_2

	 SELECT DISTINCT TOP(@MaxRecords) pat.pa_id 
	INTO #usp_SavePop2IPPCMS138v11_NQF0028_1
    FROM patients pat WITH(NOLOCK) 
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN cqm2023.PatientEncounterCodes pec WITH(NOLOCK) ON pec.PatientId=pat.pa_id
	...
	INSERT INTO cqm2023.DoctorCQMCalcPop2CMS138v11_NQF0028
    (PatientId, IPP, DoctorId, RequestId)   
    SELECT DISTINCT TOP(@MaxRecords) T1.pa_id,  1, @DoctorId, @RequestId 
	FROM #usp_SavePop2IPPCMS138v11_NQF0028_1 T1
	LEFT JOIN [cqm2023].[DoctorCQMCalcPop2CMS138v11_NQF0028] IPP WITH(NOLOCK) ON IPP.PatientId = T1.pa_id AND IPP.RequestId = @RequestId
	WHERE IPP.CalcId IS NULL  
	
	--UPDATE TOP(@MaxRecords) IPP
	--SET IPP.Denominator = 1
	SELECT TOP(@MaxRecords) IPP.CalcId
	INTO #usp_SavePop2IPPCMS138v11_NQF0028_2
	...
	SET IPP.Denominator = 1
	FROM [cqm2023].[DoctorCQMCalcPop2CMS138v11_NQF0028] IPP WITH(NOLOCK)
	INNER JOIN #usp_SavePop2IPPCMS138v11_NQF0028_2 T2 ON IPP.CalcId=T2.CalcId
