...
-- =============================================
CREATE PROCEDURE [dbo].[AMC_MEASAURE_09_4_MISSING_PATIENTS] 
...
BEGIN
	...
	SET NOCOUNT ON;
		...
		SELECT PA_FIRST, PA_LAST, PA_DOB, pa_sex, pa_address1, pa_address2, pa_city, pa_state, pa_zip,pa_ssn from patients where pa_id in ( select distinct denominator from ( SELECT CASE WHEN  ISNULL(pad.pa_bp_sys,0)>0  AND DATEDIFF(MONTH,pat.pa_dob,e.enc_date)>=12*2 THEN pad.pa_id ELSE NULL END NUMERATOR ,   e.patient_id DENOMINATOR 
...
AND (DATEDIFF(MONTH,pat.pa_dob,e.enc_date)>=12*2 OR pad.pa_bp_sys>0 )) q where q.numerator is null) order by pa_first, pa_last
