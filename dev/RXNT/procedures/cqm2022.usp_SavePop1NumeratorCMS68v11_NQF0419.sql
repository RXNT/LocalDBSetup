...
-- =============================================
CREATE   PROCEDURE [cqm2022].[usp_SavePop1NumeratorCMS68v11_NQF0419]
	@RequestId BIGINT 
AS
BEGIN
	...
	INSERT INTO cqm2022.DoctorCQMCalcPop1CMS68v11_NQF0419
	...
	FROM  cqm2022.[DoctorCQMCalcPop1CMS68v11_NQF0419] calc WITH(NOLOCK)
	...
	INNER JOIN cqm2022.PatientMedicationCodes pmc WITH(NOLOCK) ON calc.PatientId=pmc.PatientId
	...
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN cqm2022.PatientProcedureCodes ppc WITH(NOLOCK) ON pat.pa_id=ppc.PatientId AND ppc.CODE='428191000124101' AND ppc.Status='Completed' AND LEN(ISNULL(ppc.ReasonTypeCode,''))=0
	LEFT JOIN cqm2022.DoctorCQMCalcPop1CMS68v11_NQF0419 IPP WITH(NOLOCK) ON IPP.PatientId = pat.pa_id AND IPP.RequestId = @RequestId AND CAST(ppc.PerformedFromDate AS DATE)=CAST(IPP.DatePerformed AS DATE) AND IPP.Numerator =1
	...
	FROM  patients pat WITH(NOLOCK) 
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN cqm2022.[PatientInterventionCodes] ppc WITH(NOLOCK) ON pat.pa_id=ppc.PatientId AND ppc.CODE='428191000124101' AND ppc.Status='Completed' AND LEN(ISNULL(ppc.ReasonTypeCode,''))=0
	LEFT JOIN cqm2022.DoctorCQMCalcPop1CMS68v11_NQF0419 IPP WITH(NOLOCK) ON IPP.PatientId = pat.pa_id AND IPP.RequestId = @RequestId AND CAST(ppc.PerformedFromDate AS DATE)=CAST(IPP.DatePerformed AS DATE) AND IPP.Numerator =1
	WHERE dg.dc_id=@DoctorCompanyId AND ppc.DoctorId = @DoctorId 
	AND (IPP.Numerator IS NULL OR IPP.Numerator = 0)
	AND  ppc.DoctorId = @DoctorId AND ppc.PerformedFromDate BETWEEN @StartDate AND @EndDate
