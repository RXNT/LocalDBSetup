...
CREATE PROCEDURE [cqm2019].[usp_GetDoctorQRDAIII_CMS155v7_NQF0024_POP3]  
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
	 ...
	 BEGIN
		 ...
		 FROM  [cqm2019].[DoctorCQMCalcPop3CMS155v7_NQF0024] N WITH(NOLOCK)
		...
		INNER JOIN [cqm2019].[DoctorCQMCalcPop3CMS155v7_NQF0024] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		FROM patients pat WITH(NOLOCK) 
		INNER JOIN [cqm2019].[DoctorCQMCalcPop3CMS155v7_NQF0024] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		INNER JOIN  [cqm2019].SysLookupCMS155v7_NQF0024 cqm WITH(NOLOCK) ON pat.pa_sex = cqm.Code
		...
		INNER JOIN [cqm2019].[DoctorCQMCalcPop3CMS155v7_NQF0024] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
