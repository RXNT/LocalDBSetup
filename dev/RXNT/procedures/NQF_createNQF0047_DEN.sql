...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[NQF_createNQF0047_DEN]
    @doctorid BIGINT , 
	@startdate DATETIME ,
	@enddate DATETIME 
AS
...
BEGIN TRY
	select count(distinct PAT.pa_id)SM from patients PAT inner join 
                     ...
                     AND PAD.ICD9 IN (select code from NQF_CODES where IsActive=1 AND IsExclude=0 AND NQF_id='0047' AND criteriatype='POP1' AND code_type='ICD-9' ) AND PAD.enabled=1 and PAD.type=1;
END TRY
	  BEGIN CATCH
