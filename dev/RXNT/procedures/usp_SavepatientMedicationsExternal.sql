...
-- Description: <Insert/Update patient medications external data>    
-- =============================================    
CREATE PROCEDURE [dbo].[usp_SavepatientMedicationsExternal]     
...
BEGIN   
 ...
 SET	@drugId = 0
	...
	If @drugId < 1
	BEGIN
		EXEC [dbo].[addDocGroupFreeTextMeds] @drug_name,0,@dg_id,@dr_id,1
		select @drugId=medid from doc_group_freetext_meds DGF with(nolock) 
			inner join RMIID1 RM with(nolock) on DGF.drug_id = RM.MEDID
			where DGF.dg_id = @dg_id and med_medid_desc like   @drug_name 
	END
