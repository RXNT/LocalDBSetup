...
-- =============================================
CREATE PROCEDURE [dbo].[CompanyLevelDailyPrescriptionsReport]
...
BEGIN
 ...
 SET @ToDate = DATEADD(DAY,1,@FromDate)
	...
	SELECT	PD.pd_id AS RxNTId,
			...
			PD.numb_refills AS [RefillCount],
			PD.duration_amount As [Quantity],
			PD.duration_unit As [QuantityUnit],
			CAST(pd.days_supply AS INT) DaysSupply
