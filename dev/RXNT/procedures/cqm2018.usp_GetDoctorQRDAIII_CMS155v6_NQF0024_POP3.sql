...
CREATE PROCEDURE [cqm2018].[usp_GetDoctorQRDAIII_CMS155v6_NQF0024_POP3]  
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
	 ...
	 BEGIN
		 ...
		 FROM  [cqm2018].[DoctorCQMCalcPop1CMS155v6_NQF0024] N WITH(NOLOCK)
		 WHERE N.RequestId = @RequestId 
		 
		 /*Age >= 3 year(s) AND  Age < 11 year(s)*/
		 SELECT 
		 SUM(CASE WHEN N.IPP = 1 THEN 1 ELSE 0 END) as IPP, 
		 SUM(CASE WHEN N.Denominator = 1 THEN 1 ELSE 0 END) as Denominator, 
		 SUM(CASE WHEN N.DenomExclusions = 1 THEN 1 ELSE 0 END) as DenomExclusions,
		 ...
		 SUM(CASE WHEN N.NumerExclusions = 1 THEN 1 ELSE 0 END) as NumerExclusions,
		 SUM(CASE WHEN N.DenomExceptions = 1 THEN 1 ELSE 0 END) as DenomExceptions
		 FROM  [cqm2018].[DoctorCQMCalcPop3CMS155v6_NQF0024] N  WITH(NOLOCK)
		 INNER JOIN [dbo].[patients] pat WITH(NOLOCK) ON N.PatientId = pat.pa_id
		 Where N.RequestId = @RequestId AND  DATEDIFF(MONTH,pat.pa_dob,@StartDate)>=3*12 AND DATEDIFF(MONTH,pat.pa_dob,@StartDate)<11*12  
		 
		 /*Age >= 11 year(s) AND  Age < 17 year(s)*/
		 SELECT 
		 SUM(CASE WHEN N.IPP = 1 THEN 1 ELSE 0 END) as IPP, 
		 SUM(CASE WHEN N.Denominator = 1 THEN 1 ELSE 0 END) as Denominator, 
		 SUM(CASE WHEN N.DenomExclusions = 1 THEN 1 ELSE 0 END) as DenomExclusions,
		 ...
		 FROM  [cqm2018].[DoctorCQMCalcPop3CMS155v6_NQF0024] N  WITH(NOLOCK)
		 INNER JOIN [dbo].[patients] pat WITH(NOLOCK) ON N.PatientId = pat.pa_id
		 Where N.RequestId = @RequestId AND  DATEDIFF(MONTH,pat.pa_dob,@StartDate)>=11*12 AND DATEDIFF(MONTH,pat.pa_dob,@StartDate)<17*12  
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop3CMS155v6_NQF0024] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		FROM patients pat WITH(NOLOCK) 
		INNER JOIN [cqm2018].[DoctorCQMCalcPop3CMS155v6_NQF0024] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		INNER JOIN  [cqm2018].SysLookupCMS155v6_NQF0024 cqm WITH(NOLOCK) ON pat.pa_sex = cqm.Code
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop3CMS155v6_NQF0024] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
