...
-- Description:	to create a new RxNT Encounter Template For Group
-- =============================================
CREATE PROCEDURE [enc].[usp_SaveRxNTEncounterTemplateForDoctorGroup]
(
	...
	BEGIN TRY
		...
		BEGIN TRANSACTION 
			IF ISNULL(@TemplateId,0) = 0
				BEGIN
					SET @TemplateId = -1
					INSERT INTO [doc_group_enhanced_encounter_templates] 
					([dg_id],
					[added_by_dr_id],
					[enc_text],
					[chief_complaint],
			...
			ELSE 
				BEGIN
					UPDATE [doc_group_enhanced_encounter_templates] 
					SET 
						[enc_text]=@EncTextXML,
