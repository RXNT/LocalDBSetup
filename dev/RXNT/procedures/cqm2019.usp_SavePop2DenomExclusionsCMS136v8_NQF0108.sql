...
-- =============================================
CREATE PROCEDURE [cqm2019].[usp_SavePop2DenomExclusionsCMS136v8_NQF0108]
	@RequestId BIGINT  
AS
BEGIN
	...
	(
		...
		INNER JOIN cqm2019.DoctorCQMCalcPop2CMS136v8_NQF0108 IPP WITH(NOLOCK) on IPP.PatientId = pmc.PatientId AND IPP.RequestId = @RequestId
	...
	FROM cqm2019.DoctorCQMCalcPop2CMS136v8_NQF0108 IPP
	INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN (
