...
-- =============================================
CREATE PROCEDURE [dbo].[CQM_CMS69v2_NQF0421_POP2_DENEX] 
...
BEGIN
	...
	BEGIN
		...
		WHERE DATEDIFF(MONTH,PAT.pa_dob,@fromdate) BETWEEN 18*12 AND 64*12 
	...
	BEGIN
		...
		(
			(
				SELECT pa_id FROM patient_active_diagnosis WITH(NOLOCK)
				WHERE pa_id IN 
				(
					--Sub query start
					SELECT distinct PAT.pa_id FROM patients PAT WITH(NOLOCK) INNER JOIN
					(
						SELECT pa_id FROM 
						patient_procedures PP WITH(NOLOCK)
						INNER JOIN CQM_Codes cqm WITH(NOLOCK) on PP.code=cqm.code
					...
					WHERE datediff(MONTH,PAT.pa_dob,@fromdate) BETWEEN 18*12 AND 64*12 AND
					not (pa_dob like '1901-01-01') 
					--Sub query end     
				) 
				AND icd9 IN 
				(
					SELECT code FROM CQM_Codes cqm WITH(NOLOCK)
					WHERE  cqm.version=2 AND NQF_id='0421' AND cqm.code_type IN ('ICD9CM','ICD10CM') AND criteriatype='Common1' AND criteria =3  AND cqm.IsExclude=1  AND					IsActive=1
				)
			) 
			UNION
			(
					SELECT pa_id FROM patient_flag_details WITH(NOLOCK) WHERE pa_id IN 
					(
						--Sub query start
						SELECT distinct PAT.pa_id FROM patients PAT WITH(NOLOCK) INNER JOIN
						(
		...
		(
			(
				...
				(
					...
					patients PAT WITH(NOLOCK) INNER JOIN
					(
