...
-- =============================================
CREATE    PROCEDURE [cqm2023].[SavePop1DenomExceptionsCMS147v12_NQF0041]  
	@RequestId BIGINT  
AS
BEGIN
	SET NOCOUNT ON;
	...
	FROM cqm2023.DoctorCQMCalcPop1CMS147v12_NQF0041 IPP WITH(NOLOCK)
	...
	INNER JOIN cqm2023.SysLookUpCMS147v12_NQF0041 cqm WITH(NOLOCK) on cqm.code = pdc.code
	WHERE dg.dc_id=@DoctorCompanyId AND IPP.DoctorId = @DoctorId
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (2,21,23,27,28,29,30,31) AND cqm.ValueSetId = 48
	...
	INNER JOIN cqm2023.[SysLookUpCMS147v12_NQF0041] cqm WITH(NOLOCK) on cqm.code = pdc.code
	WHERE dg.dc_id=@DoctorCompanyId AND IPP.DoctorId = @DoctorId
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (2,27,28,29,30,31) AND cqm.ValueSetId = 49
	AND (pdc.PerformedFromDate BETWEEN @StartDate AND  @EndDate OR pdc.PerformedToDate BETWEEN @StartDate AND @EndDate)
	AND NOT DATEDIFF(DAY,@StartDate,pdc.PerformedFromDate) > 89 
	AND NOT DATEDIFF(DAY,@StartDate,ISNULL(pdc.PerformedToDate,DATEADD(DAY,90,@StartDate))) <= 89
	AND IPP.DenomExceptions IS NULL OR IPP.DenomExceptions = 0 AND IPP.RequestId = @RequestId
	...
	INNER JOIN cqm2023.SysLookUpCMS147v12_NQF0041 cqm WITH(NOLOCK) on cqm.code = pdc.code
	WHERE dg.dc_id=@DoctorCompanyId AND IPP.DoctorId = @DoctorId
	AND cqm.QDMCategoryId = 1 AND cqm.ValueSetId = 59 AND cqm.CodeSystemId IN (2,27,28,29,30,31)
	AND (pdc.PerformedFromDate BETWEEN @StartDate AND  @EndDate OR pdc.PerformedToDate BETWEEN @StartDate AND @EndDate)
	AND NOT DATEDIFF(DAY,@StartDate,pdc.PerformedFromDate) > 89 
	AND NOT DATEDIFF(DAY,@StartDate,ISNULL(pdc.PerformedToDate,DATEADD(DAY,90,@StartDate))) <= 89
	AND IPP.DenomExceptions IS NULL OR IPP.DenomExceptions = 0 AND IPP.RequestId = @RequestId
	...
	INNER JOIN cqm2023.[SysLookUpCMS147v12_NQF0041] cqm WITH(NOLOCK) on cqm.code = ppc.code
	WHERE dg.dc_id=@DoctorCompanyId AND IPP.DoctorId = @DoctorId 
	AND cqm.QDMCategoryId IN (2110,16) AND cqm.CodeSystemId IN (19) AND cqm.ValueSetId IN (185)
	...
	INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN cqm2023.PatientImmunizationCodes pmc WITH(NOLOCK) ON pmc.PatientId = IPP.patientId
	INNER JOIN cqm2023.[SysLookUpCMS147v12_NQF0041] cqm WITH(NOLOCK) on cqm.code = pmc.code
	WHERE dg.dc_id=@DoctorCompanyId AND IPP.DoctorId = @DoctorId
	AND cqm.QDMCategoryId = 21 AND cqm.CodeSystemId = 19 AND cqm.ValueSetId = 185
