...
-- =============================================
CREATE PROCEDURE [dbo].[CQM_CMS147v2_NQF0041_POP1_IPP] 
...
BEGIN
	...
	BEGIN
		...
		(
			...
			(
				...
				AND ((criteriatype='POP1' AND criteria=2) OR (criteriatype='Common1' AND criteria IN (1,2))) 
				AND cqm.IsActive=1 AND cqm.IsExclude=0 AND dr_id= @doctorid AND PP.date_performed BETWEEN @fromdate AND @todate
				GROUP BY pa_id HAVING COUNT(pa_id) >= 1
			)
		)PPC ON pat.pa_id=ppc.pa_id 
		WHERE datediff(month,PAT.pa_dob,CONVERT(datetime,@fromdate,101)) >= 6 AND  not (pa_dob like '1901-01-01')
	END
    ELSE IF @type = 2 --All
	BEGIN
		 select count(distinct PAT.pa_id)SM from patients PAT inner join
          (
			(select pa_id from 
				patient_procedures PP INNER JOIN CQM_Codes cqm on PP.code=cqm.code
				where cqm.version=2 AND cqm.NQF_id='0041' and cqm.code_type='CPT' 
				and criteriatype='POP1' and criteria=1
	...
	BEGIN
		select pa_race_type ,COUNT(DISTINCT pat.pa_id) SM from patients PAT inner join
          ...
          and cqm.IsActive=1 and cqm.IsExclude=0 and dr_id= @doctorid AND PP.date_performed between
          @fromdate AND @todate
          group by pa_id having count(pa_id) >= 1))PPC on PAT.pa_id=PPC.pa_id 
