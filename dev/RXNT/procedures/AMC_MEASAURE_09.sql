...
-- =============================================
CREATE PROCEDURE [dbo].[AMC_MEASAURE_09]
...
BEGIN
	...
	BEGIN
		...
		select count(distinct r.ad_id) NUMERATOR,COUNT(distinct r.pa_id) DENOMINATOR from (SELECT distinct s.pa_id, pmc.pa_id ad_id  from (SELECT DISTINCT PA_ID FROM prescriptions P WITH(NOLOCK) WHERE DR_ID=@DRID  AND PRES_PRESCRIPTION_TYPE = 1 AND PRES_VOID = 0  and pres_approved_date between @DTSTART and @DTEND UNION SELECT DISTINCT E.patient_id FROM enchanced_encounter e WITH(NOLOCK) INNER JOIN patients pat ON pat.pa_id =e.patient_id WHERE DATEDIFF(MONTH,pa_dob,enc_date)>=2*12 AND E.dr_id = @DRID and enc_date between @DTSTART and @DTEND and issigned =1) S  left outer join patient_measure_compliance pmc with(nolock)  on s.pa_id = pmc.pa_id AND pmc.rec_type=11 
	--	AND pmc.rec_date between @DTSTART and @DTEND
	) r 
	END
    SET NOCOUNT OFF;
