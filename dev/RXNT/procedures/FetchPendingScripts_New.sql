...
-- =============================================
CREATE PROCEDURE [dbo].[FetchPendingScripts_New]
	@daysBack int,@iadminCompanyId int = 0
AS
BEGIN	
	SET NOCOUNT ON;
...
SELECT dbo.doctors.dr_last_name, dbo.doctors.dr_first_name,
	 ...
	 dbo.patients.pa_id, dbo.patients.pa_first, dbo.patients.pa_middle, dbo.patients.pa_last, 
		...
		prescription_details.pd_id, drug_name, dosage, duration_amount, duration_unit, numb_refills, dbo.prescription_details.comments, use_generic, prescription_status.queued_date max_queued_date, dbo.prescriptions.SEND_COUNT
	  FROM 	dbo.prescriptions WITH(NOLOCK) INNER JOIN dbo.prescription_details WITH(NOLOCK) ON dbo.prescriptions.pres_id = dbo.prescription_details.pres_id    
		...
		dbo.doctors WITH(NOLOCK) ON prescriptions.dr_id = dbo.doctors.dr_id inner join doc_groups dg ON dbo.doctors.dg_id=dg.dg_id
	  ...
	  WHERE ADMIN_COMPANY_ID=@iadminCompanyId AND (prescriptions.pres_approved_date IS NOT NULL) AND prescriptions.pres_entry_date > GETDATE()- @daysBack AND (prescriptions.pres_void = 0)    
		...
		BEGIN
		 ...
		 dbo.patients.pa_id, dbo.patients.pa_first, dbo.patients.pa_middle, dbo.patients.pa_last, 
		  ...
		  FROM 	dbo.prescriptions WITH(NOLOCK) INNER JOIN dbo.prescription_details WITH(NOLOCK) ON dbo.prescriptions.pres_id = dbo.prescription_details.pres_id    
			...
			INNER JOIN dbo.prescription_status WITH(NOLOCK) ON dbo.prescription_details.pd_id = dbo.prescription_status.pd_id LEFT OUTER JOIN
			pharmacies WITH(NOLOCK) ON pharmacies.pharm_id = prescriptions.pharm_id
		  WHERE (prescriptions.pres_approved_date IS NOT NULL) AND prescriptions.pres_entry_date > GETDATE()- @daysBack AND (prescriptions.pres_void = 0)    
			and (prescription_status.response_type=1 or prescription_status.response_type is null)
		ORDER BY prescription_status.queued_date desc, prescriptions.pres_id
