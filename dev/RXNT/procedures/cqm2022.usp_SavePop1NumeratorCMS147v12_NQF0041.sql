...
-- =============================================
CREATE   PROCEDURE [cqm2022].[usp_SavePop1NumeratorCMS147v12_NQF0041]
	@RequestId BIGINT 
AS
BEGIN
	...
	UPDATE TOP(@MaxRowCount) cqm2022.DoctorCQMCalcPOP1CMS147v12_NQF0041
	SET Numerator = 1 
	FROM [cqm2022].[DoctorCQMCalcPOP1CMS147v12_NQF0041] pat WITH(NOLOCK)
	INNER JOIN patients pa WITH(NOLOCK) ON pa.pa_id=pat.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pa.dg_id=dg.dg_id
	INNER JOIN cqm2022.PatientProcedureCodes ppc_s WITH(NOLOCK) ON ppc_s.PatientId = pat.PatientId
	INNER JOIN cqm2022.[SysLookupCMS147v12_NQF0041] cqm WITH(NOLOCK) on cqm.Code = ppc_s.Code
	...
	INNER JOIN patients pa WITH(NOLOCK) ON pa.pa_id=pat.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pa.dg_id=dg.dg_id
	INNER JOIN cqm2022.PatientImmunizationCodes pmc WITH(NOLOCK) on pmc.PatientId = pat.PatientId
	INNER JOIN [cqm2022].[SysLookupCMS147v12_NQF0041] cqm WITH(NOLOCK) ON pmc.Code = cqm.code
