...
-- =============================================
CREATE PROCEDURE [dbo].[ImportVitasMasterCompoundDrugList]
	@dg_id int
AS
BEGIN
	...
	If exists(select top 1 * from RxNTReportUtils..vitas_formulary_Compound_list_temp)
	Begin
		PRINT 'STARTED'
		create table #temp(GCN varchar(10))
		insert into #temp
		SELECT gcn_c from RxNTReportUtils..vitas_formulary_Compound_list_temp
		...
		BEGIN
			...
			SELECT @DRUGNAME=medication from RxNTReportUtils..vitas_formulary_Compound_list_temp 
			where gcn_c = @GCN
			
			-- If GCN does not exists ADD the new drug as freetext
			If not exists 
				(select * from RMIID1 with(nolock) where GCN_STRING =@GCN)
			BEGIN
				If(LEN(@DRUGNAME)) > 0
				BEGIN
