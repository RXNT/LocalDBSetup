...
-- =============================================
CREATE PROCEDURE [cqm2019].[usp_GetCQMMeasureInfo] 
	@NQFNumber VARCHAR(MAX) 
AS
BEGIN

	DECLARE @MeasureId BIGINT = 0
	SELECT @MeasureId = MeasureInfoId FROM cqm2019.SysLookupCqmMeasuresInfo WITH(NOLOCK) WHERE NQFNumber = @NQFNumber
	
	SELECT MeasureInfoId, MeasureTitle, ReferenceId, Description, MeasureNumber, NQFNumber
	FROM cqm2019.SysLookupCqmMeasuresInfo WITH(NOLOCK)
	...
	FROM cqm2019.SysLookupCqmMeasurePopulationInfo WITH(NOLOCK)
	WHERE MeasureInfoId = @MeasureId
	...
	FROM cqm2019.SysLookupCqmMeasureStratumInfo WITH(NOLOCK)
	WHERE MeasureInfoId = @MeasureId 
