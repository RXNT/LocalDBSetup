...
-- Description:	Search Doctors by DoctorGroupId (referred PatientDashboard)- Added ogic to search by name 
-- Modified By: 
-- Modified Date: 
-- =============================================
CREATE PROCEDURE [enc].[usp_SearchDoctors] 
...
BEGIN
	 SELECT D.DR_FIRST_NAME,D.epcs_enabled,D.dr_enabled,loginlock, D.DR_PREFIX, D.DR_SUFFIX, d.hipaa_agreement_acptd,
      ...
      WHERE  (D.DR_LAST_NAME LIKE @Name+'%' OR D.DR_FIRST_NAME LIKE '%'+@Name+'%' )  AND D.DG_ID = @DoctorGroupId  AND D.DR_ENABLED = 1 and loginlock = 0 
