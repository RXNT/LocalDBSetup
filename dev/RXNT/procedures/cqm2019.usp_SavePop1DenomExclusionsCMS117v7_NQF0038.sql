...
-- =============================================
CREATE PROCEDURE [cqm2019].[usp_SavePop1DenomExclusionsCMS117v7_NQF0038]
	@RequestId BIGINT  
AS
BEGIN
	...
	FROM [cqm2019].[DoctorCQMCalcPop1CMS117v7_NQF0038] IPP
	INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN cqm2019.PatientInterventionCodes pic ON pic.PatientId = IPP.PatientId
	INNER JOIN [cqm2019].[SysLookupCMS117v7_NQF0038] cqm on cqm.code = pic.code
	INNER JOIN cqm2019.PatientEncounterCodes pec ON pec.EncounterId = pic.EncounterId
