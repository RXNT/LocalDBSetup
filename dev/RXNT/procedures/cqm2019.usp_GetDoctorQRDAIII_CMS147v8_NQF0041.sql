...
-- =============================================
-- Author:		Niyaz
-- Create date: 11-DEC-2018
-- Description:	To get the Doctor CQM request status 
-- =============================================
 
CREATE PROCEDURE [cqm2019].[usp_GetDoctorQRDAIII_CMS147v8_NQF0041]  
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
	 ...
	 SELECT @RequestId = MAX(RequestId) FROM  [cqm2019].[DoctorCQMCalculationRequest] R 
	 ...
	 BEGIN
		 ...
		 FROM  [cqm2019].[DoctorCQMCalcPop1CMS147v8_NQF0041] N WITH(NOLOCK)
		...
		INNER JOIN [cqm2019].[DoctorCQMCalcPop1CMS147v8_NQF0041] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		FROM patients pat WITH(NOLOCK) 
		INNER JOIN [cqm2019].[DoctorCQMCalcPop1CMS147v8_NQF0041] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		INNER JOIN  [cqm2019].SysLookupCMS147v8_NQF0041 cqm WITH(NOLOCK) ON pat.pa_sex = cqm.Code
		...
		INNER JOIN [cqm2019].[DoctorCQMCalcPop1CMS147v8_NQF0041] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
		WHERE N.RequestId = @RequestId 
