...
-- =============================================
CREATE   PROCEDURE [cqm2022].[usp_SaveQDMCMS153v10_NQF0033_Diagnosis]
	@RequestId BIGINT
AS
BEGIN
	...
	INNER JOIN cqm2022.SysLookUpCMS153v10_NQF0033 cqm WITH(NOLOCK) ON pad.icd9 = cqm.code 
	...
	FROM patient_active_diagnosis pad WITH(NOLOCK)
	INNER JOIN patients pat WITH(NOLOCK) ON pad.pa_id=pat.pa_id
	INNER JOIN cqm2022.SysLookUpCMS153v10_NQF0033 cqm  WITH(NOLOCK)ON pad.icd10 = cqm.code 
	LEFT JOIN cqm2022.PatientDiagnosisCodes pdc WITH(NOLOCK) on pad.pad = pdc.DiagnosisId AND pdc.EncounterId = EncounterId 
	WHERE pad.added_by_dr_id = @DoctorId
	AND (pad.date_added BETWEEN @StartDate AND @EndDate OR pad.status_date BETWEEN @StartDate AND @EndDate) 
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (2,21,23,27,28,29,30,31) -- Done 
	...
	FROM patient_active_diagnosis pad WITH(NOLOCK)
	INNER JOIN patients pat WITH(NOLOCK) ON pad.pa_id=pat.pa_id
	INNER JOIN cqm2022.SysLookUpCMS153v10_NQF0033 cqm WITH(NOLOCK) ON pad.snomed_code = cqm.code 
	LEFT JOIN cqm2022.PatientDiagnosisCodes pdc WITH(NOLOCK) on pad.pad = pdc.DiagnosisId AND pdc.EncounterId = EncounterId 
	WHERE pad.added_by_dr_id = @DoctorId
	AND (pad.date_added BETWEEN @StartDate AND @EndDate OR pad.status_date BETWEEN @StartDate AND @EndDate) 
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (2,21,23,27,28,29,30,31) -- Done 
