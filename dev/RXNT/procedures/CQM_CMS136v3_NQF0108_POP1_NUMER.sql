...
-- =============================================
CREATE PROCEDURE [dbo].[CQM_CMS136v3_NQF0108_POP1_NUMER] 
...
BEGIN
	...
	BEGIN
        ...
        INNER JOIN cqm_codes cqm_pad_excl 
        ON (cqm_pad_excl.code = pad_excl.icd9 
			OR pad_excl.icd10=cqm_pad_excl.CODE 
			OR pad_excl.snomed_code=cqm_pad_excl.CODE
			)
        INNER JOIN 	patient_procedures PP_num ON PP_num.pa_id=pres_ipp.pa_id 
        INNER JOIN cqm_codes cqm_pp_num ON cqm_pp_num.code = PP_num.code 
        ...
        AND cqm_pp_num.version=2 AND cqm_pp_num.nqf_id='0108' 
        AND cqm_pp_num.isactive=1 AND cqm_pp_num.code_type IN ('CPT','SNOMEDCT')  
        AND cqm_pp_num.criteriatype='COMMON1' AND cqm_pp_ipp.isexclude=0 
        
        AND cqm_pp_ipp.criteria IN (3,7,8,9,8,10,11,12,13,14,15) 
        ...
        AND  cqm_pp_num.version=2 AND cqm_pp_num.nqf_id='0108' 
        AND cqm_pp_num.isactive=1 AND cqm_pp_num.code_type IN ('CPT','SNOMEDCT')  
        AND cqm_pp_num.criteriatype='COMMON1' AND cqm_pp_num.isexclude=0 
        AND cqm_pp_num.criteria IN (3,7,8,9,10,11,12,13,14,15) 
        AND DATEDIFF(day,pam_excl.date_start,pres_ipp.pres_approved_date) BETWEEN  1 AND 120
        AND pres_ipp.pres_void = 0 AND pd_ipp.history_enabled = 1
        AND DATEDIFF(day,pres_ipp.pres_approved_date,PP_num.date_performed) BETWEEN  1 AND 30 
	...
	BEGIN
        ...
        AND cqm_pad_excl.version=2 AND cqm_pad_excl.nqf_id='0108' AND cqm_pad_excl.isactive=1 AND cqm_pad_excl.code_type IN ('ICD9CM','SNOMEDCT','ICD10CM')  AND cqm_pad_excl.criteriatype='COMMON1' AND cqm_pad_excl.isexclude=1 AND cqm_pad_excl.criteria = 4
        AND cqm_pp_num.version=2 AND cqm_pp_num.nqf_id='0108' AND cqm_pp_num.isactive=1 AND cqm_pp_num.code_type IN ('CPT','SNOMEDCT')  AND cqm_pp_num.criteriatype='COMMON1' AND cqm_pp_ipp.isexclude=0 AND cqm_pp_ipp.criteria IN (3,7,8,9,8,10,11,12,13,14,15) 
        AND pres_ipp.pres_void = 0 AND pd_ipp.history_enabled = 1
        ...
        INNER JOIN cqm_codes cqm_pp_ipp ON cqm_pp_ipp.code = PP_ipp.code 
        INNER JOIN patient_active_meds pam_excl ON pres_ipp.pa_id!=pam_excl.pa_id
        INNER JOIN revdel0 R1_excl WITH(NOLOCK) ON CAST(pam_excl.drug_id AS VARCHAR)=R1_excl.evd_fdb_vocab_id  
        INNER JOIN REVDVT0 R2_excl WITH(NOLOCK) ON  r1_excl.evd_ext_vocab_type_id=R2_excl.evd_vocab_type_id   
        INNER JOIN cqm_codes cqm_pres_excl ON cqm_pres_excl.code =CAST(R1_excl.EVD_EXT_VOCAB_ID AS VARCHAR)
        ...
        AND cqm_pam_ipp.version=2 AND cqm_pam_ipp.nqf_id='0108' AND cqm_pam_ipp.isactive=1 AND cqm_pam_ipp.code_type ='RXNORM'  AND cqm_pam_ipp.criteriatype='COMMON1' AND cqm_pam_ipp.isexclude=0 AND cqm_pam_ipp.criteria=1 
        AND cqm_pp_ipp.version=2 AND cqm_pp_ipp.nqf_id='0108' AND cqm_pp_ipp.isactive=1 AND cqm_pp_ipp.code_type IN ('CPT','SNOMEDCT')  AND cqm_pp_ipp.criteriatype='COMMON1' AND cqm_pp_ipp.isexclude=0 AND cqm_pp_ipp.criteria=3 
        AND cqm_pres_excl.version=2 AND cqm_pres_excl.nqf_id='0108' AND cqm_pres_excl.isactive=1 AND cqm_pres_excl.code_type ='RXNORM'  AND cqm_pres_excl.criteriatype='COMMON1' AND cqm_pres_excl.isexclude=0 AND cqm_pres_excl.criteria = 1
        AND  cqm_pp_num.version=2 AND cqm_pp_num.nqf_id='0108' AND cqm_pp_num.isactive=1 AND cqm_pp_num.code_type IN ('CPT','SNOMEDCT')  AND cqm_pp_num.criteriatype='COMMON1' AND cqm_pp_num.isexclude=0 AND cqm_pp_num.criteria IN (3,7,8,9,10,11,12,13,14,15) 
        AND DATEDIFF(day,pam_excl.date_start,pres_ipp.pres_approved_date) BETWEEN  1 AND 120
        AND pres_ipp.pres_void = 0 AND pd_ipp.history_enabled = 1
        AND DATEDIFF(day,pres_ipp.pres_approved_date,PP_num.date_performed) BETWEEN  1 AND 30 
        )PPC on PAT.pa_id=PPC.pa_id 
