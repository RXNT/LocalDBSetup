...
-- =============================================
CREATE PROCEDURE [dbo].[CQM_CMS138v2_NQF0028_POP1_DENEXCEP] 
...
BEGIN
	...
	BEGIN
        ...
        INNER JOIN 
           (SELECT pa_id FROM patient_procedures PP 
			...
			and cqm.nqf_id='0028' AND cqm.code_type='CPT'
           ...
           INNER JOIN patient_procedures pp_exc ON pp_exc.pa_id = pat.pa_id  
           INNER JOIN cqm_codes cqm_exc ON pp_exc.code=cqm_exc.code
           WHERE datediff(month,PAT.pa_dob,CONVERT(datetime,@fromdate,101)) >= 18*12 
           AND not (pa_dob like '1901-01-01')
           AND cqm_exc.version=2 AND cqm_exc.nqf_id='0028' 
           AND  cqm_exc.criteriatype='EXCL' AND cqm_exc.isexclude=1 
           AND pp_exc.date_performed between  @fromdate AND @todate
	END
	...
	BEGIN
		 SELECT pa_race_type ,COUNT(DISTINCT pat.pa_id) SM  FROM patients PAT INNER JOIN 
           ...
           AND cqm_exc.version=2 AND cqm_exc.nqf_id='0028' AND  cqm_exc.criteriatype='EXCL' AND cqm_exc.isexclude=1 AND pp_exc.date_performed between  @fromdate AND @todate
