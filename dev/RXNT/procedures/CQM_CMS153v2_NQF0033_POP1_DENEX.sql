...
-- =============================================
CREATE PROCEDURE [dbo].[CQM_CMS153v2_NQF0033_POP1_DENEX] 
...
BEGIN
	...
	BEGIN
         ...
         SELECT pp.pa_id,PP.date_performed from 
         patient_procedures PP  INNER JOIN CQM_Codes cqm_pp on PP.code=cqm_pp.code
         ...
         INNER JOIN lab_test_lists LTL_excl1 with(nolock) 
         ON PLO_excl1.lab_test_id=LTL_excl1.lab_test_id  
         left join pem_links plk_excl1 with(nolock) 
         on plk_excl1.pem_title = LTL_excl1.lab_test_name  
         ...
         INNER JOIN lab_test_lists LTL_excl2 with(nolock) 
         ON PLO_excl2.lab_test_id=LTL_excl2.lab_test_id  
         left join pem_links plk_excl2 with(nolock) 
         on plk_excl2.pem_title = LTL_excl2.lab_test_name  
         ...
         AND cqm_excl1.version=2 AND cqm_excl1.NQF_id='0033' 
         AND cqm_excl1.code_type = 'LOINC'  AND cqm_excl1.criteriatype='DENOM'  
         AND cqm_excl1.IsActive=1 AND cqm_excl1.IsExclude=1  
         AND cqm_excl1.criteria=1 
         AND ltl_excl1.test_type=0 AND PLO_excl1.order_date between @fromdate AND @todate 
         AND cqm_excl2.version=2 AND cqm_excl2.NQF_id='0033' 
         AND cqm_excl2.code_type = 'LOINC'  AND cqm_excl2.criteriatype='DENOM'  
         AND cqm_excl2.IsActive=1 AND cqm_excl2.IsExclude=1  
         AND cqm_excl2.criteria=2
         AND ltl_excl2.test_type=1 
         AND DATEDIFF(DAY,PLO_excl1.order_date,PLO_excl2.order_date) BETWEEN 0 AND 7
         AND pp.dr_id= @doctorid AND PP.date_performed between @fromdate AND @todate 
         GROUP BY pp.pa_id,PP.date_performed having COUNT(pp.pa_id) > 0
