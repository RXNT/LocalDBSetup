...
-- =============================================
CREATE   PROCEDURE [cqm2022].[usp_SavePop1DenomExclusionsCMS69v11_NQF0421]
	@RequestId BIGINT  
AS
BEGIN
	...
	INNER JOIN (
		...
		cqm2022.PatientInterventionCodes ppc WITH(NOLOCK)
		INNER JOIN patients pat WITH(NOLOCK) ON ppc.PatientId=pat.pa_id
		INNER JOIN cqm2022.[SysLookUpCMS69v11_NQF0421] cqm WITH(NOLOCK) on cqm.code = ppc.code
		INNER JOIN cqm2022.PatientEncounterCodes pec WITH(NOLOCK) ON pec.EncounterId = ppc.EncounterId
		...
		cqm2022.PatientDiagnosisCodes pdc WITH(NOLOCK)
		INNER JOIN cqm2022.[SysLookUpCMS69v11_NQF0421] cqm  WITH(NOLOCK) on cqm.code = pdc.code
