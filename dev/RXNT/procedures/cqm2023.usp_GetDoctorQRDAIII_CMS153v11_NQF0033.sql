...
CREATE   PROCEDURE [cqm2023].[usp_GetDoctorQRDAIII_CMS153v11_NQF0033]  
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
	 ...
	 BEGIN
		 ...
		 FROM  [cqm2023].[DoctorCQMCalcPop1CMS153v11_NQF0033] N WITH(NOLOCK)
		 WHERE N.RequestId = @RequestId 
		 
		 /*Age >= 16 year(s) AND  Age <= 19 year(s)*/
		 SELECT 
		 SUM(CASE WHEN N.IPP = 1 THEN 1 ELSE 0 END) as IPP, 
		 SUM(CASE WHEN N.Denominator = 1 THEN 1 ELSE 0 END) as Denominator, 
		 SUM(CASE WHEN N.DenomExclusions = 1 THEN 1 ELSE 0 END) as DenomExclusions,
		 ...
		 Where N.RequestId = @RequestId AND  DATEDIFF(MONTH,pat.pa_dob,@StartDate)>=16*12 AND DATEDIFF(MONTH,pat.pa_dob,@StartDate)<=19*12  
		 
		 /*Age >= 20 year(s) AND  Age < 24 year(s)*/
		 SELECT 
		 SUM(CASE WHEN N.IPP = 1 THEN 1 ELSE 0 END) as IPP, 
		 SUM(CASE WHEN N.Denominator = 1 THEN 1 ELSE 0 END) as Denominator, 
		 SUM(CASE WHEN N.DenomExclusions = 1 THEN 1 ELSE 0 END) as DenomExclusions,
		 ...
		 FROM  [cqm2023].[DoctorCQMCalcPop1CMS153v11_NQF0033] N  WITH(NOLOCK)
		 INNER JOIN [dbo].[patients] pat WITH(NOLOCK) ON N.PatientId = pat.pa_id
		 Where N.RequestId = @RequestId AND  DATEDIFF(MONTH,pat.pa_dob,@StartDate)>=20*12 AND DATEDIFF(MONTH,pat.pa_dob,@StartDate)<24*12  
		...
		INNER JOIN [cqm2023].[DoctorCQMCalcPop1CMS153v11_NQF0033] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		FROM patients pat WITH(NOLOCK) 
		INNER JOIN [cqm2023].[DoctorCQMCalcPop1CMS153v11_NQF0033] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		INNER JOIN  [cqm2023].SysLookupCMS153v11_NQF0033 cqm WITH(NOLOCK) ON pat.pa_sex = cqm.Code
		...
		INNER JOIN [cqm2023].[DoctorCQMCalcPop1CMS153v11_NQF0033] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
