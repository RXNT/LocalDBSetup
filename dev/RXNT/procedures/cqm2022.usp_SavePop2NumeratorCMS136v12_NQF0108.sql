...
CREATE   PROCEDURE [cqm2022].[usp_SavePop2NumeratorCMS136v12_NQF0108]
	@RequestId BIGINT  
AS
BEGIN
	...
	(
		...
		LEFT JOIN cqm2022.DoctorCQMCalcPop2CMS136v12_NQF0108 IPP  WITH(NOLOCK) on IPP.PatientId = pmc.PatientId AND IPP.RequestId = @RequestId
	...
	SET Numerator = 1
	FROM cqm2022.DoctorCQMCalcPOP2CMS136v12_NQF0108 IPP WITH(NOLOCK)
	INNER JOIN cqm2022.PatientEncounterCodes pec  WITH(NOLOCK) on pec.PatientId = IPP.PatientId
	INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN [cqm2022].[SysLookupCMS136v12_NQF0108] cqm  WITH(NOLOCK) ON pec.Code = cqm.code	 
