...
CREATE   PROCEDURE [cqm2022].[usp_GetDoctorQRDAIII_CMS68v11_NQF0419]  
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
		...
		FROM  [cqm2022].[DoctorCQMCalcPop1CMS68v11_NQF0419] N WITH(NOLOCK)
		...
		INNER JOIN [cqm2022].[DoctorCQMCalcPop1CMS68v11_NQF0419] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		FROM patients pat WITH(NOLOCK) 
		INNER JOIN [cqm2022].[DoctorCQMCalcPop1CMS68v11_NQF0419] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		INNER JOIN  [cqm2022].SysLookupCMS68v11_NQF0419 cqm WITH(NOLOCK) ON pat.pa_sex = cqm.Code
		...
		SUM(CASE WHEN N.NumerExclusions = 1 THEN 1 ELSE 0 END) as NumerExclusions,
		SUM(CASE WHEN N.DenomExceptions = 1 THEN 1 ELSE 0 END) as DenomExceptions
		FROM  [cqm2022].vwpatientpayers pyr WITH(NOLOCK)
		INNER JOIN [cqm2022].[DoctorCQMCalcPop1CMS68v11_NQF0419] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
