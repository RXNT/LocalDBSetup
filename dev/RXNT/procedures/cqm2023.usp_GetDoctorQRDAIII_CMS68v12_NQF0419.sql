...
CREATE   PROCEDURE [cqm2023].[usp_GetDoctorQRDAIII_CMS68v12_NQF0419]  
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
		...
		FROM  [cqm2023].[DoctorCQMCalcPop1CMS68v12_NQF0419] N WITH(NOLOCK)
		...
		INNER JOIN [cqm2023].[DoctorCQMCalcPop1CMS68v12_NQF0419] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		FROM patients pat WITH(NOLOCK) 
		INNER JOIN [cqm2023].[DoctorCQMCalcPop1CMS68v12_NQF0419] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		INNER JOIN  [cqm2023].SysLookupCMS68v12_NQF0419 cqm WITH(NOLOCK) ON pat.pa_sex = cqm.Code
		...
		SUM(CASE WHEN N.NumerExclusions = 1 THEN 1 ELSE 0 END) as NumerExclusions,
		SUM(CASE WHEN N.DenomExceptions = 1 THEN 1 ELSE 0 END) as DenomExceptions
		FROM  [cqm2023].vwpatientpayers pyr WITH(NOLOCK)
		INNER JOIN [cqm2023].[DoctorCQMCalcPop1CMS68v12_NQF0419] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
