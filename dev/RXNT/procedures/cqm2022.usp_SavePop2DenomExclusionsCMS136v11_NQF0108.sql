...
-- =============================================
CREATE   PROCEDURE [cqm2022].[usp_SavePop2DenomExclusionsCMS136v11_NQF0108]
	@RequestId BIGINT  
AS
BEGIN
	...
	(
		...
		INNER JOIN cqm2022.DoctorCQMCalcPop2CMS136v11_NQF0108 IPP WITH(NOLOCK) on IPP.PatientId = pmc.PatientId AND IPP.RequestId = @RequestId
	...
	FROM cqm2022.DoctorCQMCalcPop2CMS136v11_NQF0108 IPP
	INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN (
		SELECT DISTINCT PatientId 
		FROM cqm2022.PatientDiagnosisCodes pdc WITH(NOLOCK)
		INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=pdc.PatientId
		INNER JOIN cqm2022.SysLookUpCMS136v11_NQF0108 cqm WITH(NOLOCK) on cqm.code = pdc.code	
		...
		SELECT DISTINCT PatientId 
		FROM cqm2022.PatientInterventionCodes pic WITH(NOLOCK)
		INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=pic.PatientId
		INNER JOIN cqm2022.SysLookUpCMS136v11_NQF0108 cqm  WITH(NOLOCK)on cqm.code = pic.code	
