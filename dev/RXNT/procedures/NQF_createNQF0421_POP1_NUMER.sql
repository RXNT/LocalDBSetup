...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[NQF_createNQF0421_POP1_NUMER]
    @dgid VARCHAR(15),
...
BEGIN TRY
	...
	set @query ='select count(distinct PAT.pa_id)SM from patients PAT inner join ((select PV.pa_id from patient_vitals PV inner join patient_procedures PP on 
	                 PV.pa_id=PP.pa_id where dg_id ='+@dgid+'
                     and PV.record_date between DATEADD(month,-6,PP.date_performed) and 
                     PP.date_performed and pa_bmi >= 22 and pa_bmi < 30 AND PV.pa_id in ('+@PatientIds+') group by PV.pa_id 
                 ...
                 PV.record_date between DATEADD(month,-6,PP.date_performed) and PP.date_performed and 
                  not(pa_bmi >= 22 and pa_bmi < 30) and PAD.icd9 in (select CODE from NQF_codes NQF where NQF.NQF_id=0421 
