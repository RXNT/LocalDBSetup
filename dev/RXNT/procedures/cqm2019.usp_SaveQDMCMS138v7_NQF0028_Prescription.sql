...
-- Description:	To populate QDM tables for the measure CMS138v7_NQF0028
-- =============================================
CREATE PROCEDURE [cqm2019].[usp_SaveQDMCMS138v7_NQF0028_Prescription] 
	@RequestId BIGINT
AS
BEGIN
	...
	SELECT TOP(@MaxRowsCount) pres_num.pa_id, [cqm2019].[FindClosestEncounter](pres_num.pres_approved_date, pres_num.pa_id, @DoctorId) AS EncounterId,
	...
	INNER JOIN [cqm2019].[SysLookupCMS138v7_NQF0028] cqm WITH(NOLOCK) on cqm.code = vwRx.RxNormCode
	LEFT JOIN [cqm2019].[PatientMedicationCodes] pmc WITH(NOLOCK) on pmc.EncounterId = EncounterId AND pmc.PrescriptionId = pres_num.pres_id AND pmc.Code = cqm.Code AND pmc.PatientId =  pres_num.pa_id
	...
	AND cqm.QDMCategoryId = 5 AND cqm.CodeSystemId = 7 AND cqm.ValueSetId = 166 
