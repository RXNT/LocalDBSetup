...
-- =============================================
CREATE    PROCEDURE [cqm2023].[usp_SavePop1DenomExclusionsCMS69v11_NQF0421]
	@RequestId BIGINT  
AS
BEGIN
	...
	INNER JOIN (
		...
		cqm2023.PatientInterventionCodes ppc WITH(NOLOCK)
		INNER JOIN patients pat WITH(NOLOCK) ON ppc.PatientId=pat.pa_id
		INNER JOIN cqm2023.[SysLookUpCMS69v11_NQF0421] cqm WITH(NOLOCK) on cqm.code = ppc.code
		INNER JOIN cqm2023.PatientEncounterCodes pec WITH(NOLOCK) ON pec.EncounterId = ppc.EncounterId
		...
		cqm2023.PatientDiagnosisCodes pdc WITH(NOLOCK)
		INNER JOIN cqm2023.[SysLookUpCMS69v11_NQF0421] cqm  WITH(NOLOCK) on cqm.code = pdc.code
