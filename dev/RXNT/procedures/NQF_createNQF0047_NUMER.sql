...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[NQF_createNQF0047_NUMER]
    @doctorid BIGINT , 
	@startdate DATETIME ,
	@enddate DATETIME 
AS
...
BEGIN TRY
	select count(distinct PAT.pa_id)SM from patients PAT inner join 
                     ...
                     INNER JOIN patient_Active_meds PAM on PAM.pa_id=PAT.pa_id where datediff(month,PAT.pa_dob,CONVERT(datetime,@startdate,101)) >= 5*12 and
                     ...
                     AND PAM.drug_id in (select code from NQF_CODES where IsActive=1 AND IsExclude=0 AND NQF_id='0047' AND criteriatype='NUM1' AND code_type='MED');
END TRY
	  BEGIN CATCH
