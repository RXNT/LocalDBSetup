...
CREATE PROCEDURE [cqm2018].[usp_GetDoctorQRDAIII_CMS138v6_NQF0028]
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
	 ...
	 SELECT @RequestId = MAX(RequestId) FROM  [cqm2018].[DoctorCQMCalculationRequest] R 
	 ...
	 BEGIN
		 ...
		 FROM  [cqm2018].[DoctorCQMCalcPop1CMS138v6_NQF0028] N WITH(NOLOCK)
		 WHERE N.RequestId = @RequestId 
		 
		 --Race Wise
		 
		SELECT vwRace.RaceId AS Code,SUM(CASE WHEN N.IPP = 1 THEN 1 ELSE 0 END) as IPP, 
		SUM(CASE WHEN N.Denominator = 1 THEN 1 ELSE 0 END) as Denominator, 
		SUM(CASE WHEN N.DenomExclusions = 1 THEN 1 ELSE 0 END) as DenomExclusions,
		SUM(CASE WHEN N.Numerator = 1 THEN 1 ELSE 0 END) as Numerator,
		SUM(CASE WHEN N.NumerExclusions = 1 THEN 1 ELSE 0 END) as NumerExclusions,
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop1CMS138v6_NQF0028] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		FROM patients pat WITH(NOLOCK) 
		INNER JOIN [cqm2018].[DoctorCQMCalcPop1CMS138v6_NQF0028] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		INNER JOIN  [cqm2018].[SysLookupCMS138v6_NQF0028] cqm WITH(NOLOCK) ON pat.pa_sex = cqm.Code
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop1CMS138v6_NQF0028] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
		WHERE N.RequestId = @RequestId 
