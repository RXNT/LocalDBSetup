...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE  [support].[AddVitasDocGroupFreeTextMeds] 
@drug_name VARCHAR(200),
@drug_level INT,
@dg_id BIGINT,
@added_by_dr_id BIGINT,
@drug_category INT,
@form_status INT=NULL,-- Preferred Status 1 -> Non - Preferred  ( OFF Formulary)numbericFormularyStatus == 2 -> Preferred ( ON Formulary)
@GCN varchar(10) = null,
...
IF NOT EXISTS(SELECT TOP 1 * FROM Formularies..[RxHub_VITAS_FORM_VXFORM] WHERE GCN=@GCN)
BEGIN
	
	INSERT INTO Formularies..[RxHub_VITAS_FORM_VXFORM] 
	(source_ndc,form_status,rel_value,Text,GCN)
	VALUES('',@form_status,-1,NULL,@GCN)
END
...
BEGIN 
	...
	BEGIN
		INSERT INTO doc_group_freetext_meds
			(dg_id,added_by_dr_id,added_date,drug_name,drug_id,drug_category,is_active)
		VALUES
			(@dg_id,@added_by_dr_id,GETDATE(),@drug_name,@drug_id,@drug_category,1)		
	END 
