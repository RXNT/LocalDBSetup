...
CREATE    PROCEDURE [cqm2023].[usp_SavePop2NumeratorCMS138v11_NQF0028]
	@RequestId BIGINT 
AS
BEGIN
	...
	FROM cqm2023.DoctorCQMCalcPop2CMS138v11_NQF0028 IPP WITH(NOLOCK)
	...
	WHERE dg.dc_id=@DoctorCompanyId AND IPP.RequestId = @RequestId AND cqm_r.QDMCategoryId = 17 AND cqm_r.CodeSystemId = 25 AND cqm_r.ValueSetId = 167
	AND prc.PerformedFromDate BETWEEN @measureStartDate AND @EndDate AND pfd.flag_id IN (-5,-4,-2,-1,5)
	
	
	UPDATE TOP(@MaxRows) num
	SET Numerator = 1 
	FROM [cqm2023].[DoctorCQMCalcPop2CMS138v11_NQF0028] num WITH(NOLOCK)
