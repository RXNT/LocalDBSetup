...
CREATE PROCEDURE [dbo].[SelectVitasPatientCML]
@pa_id int
AS
BEGIN
...
0 ETC_ID, '' ETC_NAME, 
A.pres_entry_date DATE_ADDED, COMPOUND, B.COMMENTS, B.pd_id from_pd_id, NULL AS record_source,
B.pd_id,
...
B.drug_indication,
A.dr_id,NULL discharge_date
FROM 
prescriptions A WITH(NOLOCK)	
INNER JOIN prescription_details B WITH(NOLOCK) ON A.pres_id = B.pres_id
INNER JOIN prescription_external_info PEI with(nolock) on A.pres_id = PEI.pres_id
...
AND (batch_id IS NULL or response_status = 'Failed' Or external_source_syncdate IS NULL)
