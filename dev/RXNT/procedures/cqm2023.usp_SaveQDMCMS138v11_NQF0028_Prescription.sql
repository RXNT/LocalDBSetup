...
-- =============================================
CREATE    PROCEDURE [cqm2023].[usp_SaveQDMCMS138v11_NQF0028_Prescription] 
	@RequestId BIGINT
AS
BEGIN
	...
	SELECT TOP(@MaxRowsCount) pres_num.pa_id, [cqm2023].[FindClosestEncounter](pres_num.pres_approved_date, pres_num.pa_id, @DoctorId) AS EncounterId,
	...
	INNER JOIN [cqm2023].[SysLookupCMS138v11_NQF0028] cqm WITH(NOLOCK) on cqm.code = vwRx.RxNormCode
	LEFT JOIN [cqm2023].[PatientMedicationCodes] pmc WITH(NOLOCK) on pmc.EncounterId = EncounterId AND pmc.PrescriptionId = pres_num.pres_id AND pmc.Code = cqm.Code AND pmc.PatientId =  pres_num.pa_id
