...
-- Description:	To save the POP 2 numerator for the measure
-- =============================================

CREATE   PROCEDURE [cqm2022].[usp_SavePop2NumeratorCMS136v11_NQF0108]
	@RequestId BIGINT  
AS
BEGIN
	...
	(
		...
		INNER JOIN cqm2022.SysLookupCMS136v11_NQF0108 cqm  WITH(NOLOCK) ON cqm.code = pmc.Code		
		LEFT JOIN cqm2022.DoctorCQMCalcPop2CMS136v11_NQF0108 IPP  WITH(NOLOCK) on IPP.PatientId = pmc.PatientId AND IPP.RequestId = @RequestId
	...
	SET Numerator = 1
	FROM cqm2022.DoctorCQMCalcPOP2CMS136v11_NQF0108 IPP WITH(NOLOCK)
	INNER JOIN cqm2022.PatientEncounterCodes pec  WITH(NOLOCK) on pec.PatientId = IPP.PatientId
	INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN [cqm2022].[SysLookupCMS136v11_NQF0108] cqm  WITH(NOLOCK) ON pec.Code = cqm.code	 
	INNER JOIN InitialADHDMedication M ON M.pa_id=IPP.PatientId	
	INNER JOIN cqm2022.PatientEncounterCodes pec1  WITH(NOLOCK) on pec1.PatientId = IPP.PatientId
	...
	AND DATEDIFF(Day,pec1.PerformedFromDate, M.PerformedFromDate) Between 31 AND 300
