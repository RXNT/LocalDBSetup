...
-- =============================================
CREATE PROCEDURE [dbo].[GenerateRxErrorList_test]	
...
BEGIN	
	...
	IF @bUseDeliveryType = 1	
		...
		FROM PRESCRIPTIONS (NOLOCK) P INNER JOIN PRESCRIPTION_DETAILS (NOLOCK)  PD ON P.PRES_ID = PD.PRES_ID
		INNER JOIN PHARMACIES (NOLOCK) PH ON P.PHARM_ID = PH.PHARM_ID INNER JOIN
		PATIENTS PA ON P.PA_ID = PA.PA_ID INNER JOIN DOCTORS (NOLOCK) DR ON P.DR_ID = DR.DR_ID
		WHERE P.PRES_ID IN (
		select a.pres_id from 
		(select p.pres_id, min(case when pt.response_type is null then -1 else pt.response_type end) response from prescriptions (NOLOCK) p 
		inner join prescription_details (NOLOCK) pd on
	...
	ELSE	
		...
		FROM PRESCRIPTIONS (NOLOCK) P INNER JOIN PRESCRIPTION_DETAILS (NOLOCK) PD ON P.PRES_ID = PD.PRES_ID
		INNER JOIN PHARMACIES (NOLOCK) PH ON P.PHARM_ID = PH.PHARM_ID INNER JOIN
		PATIENTS (NOLOCK) PA ON P.PA_ID = PA.PA_ID INNER JOIN DOCTORS DR ON P.DR_ID = DR.DR_ID
		WHERE P.PRES_ID IN (
		select a.pres_id from 
		(select p.pres_id, min(case when pt.response_type is null then -1 else pt.response_type end) response from prescriptions (NOLOCK) p 
		inner join prescription_details (NOLOCK) pd on
		p.pres_id = pd.pres_id inner join prescription_transmittals (NOLOCK) pt
