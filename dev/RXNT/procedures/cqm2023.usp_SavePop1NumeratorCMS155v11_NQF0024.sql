...
CREATE    PROCEDURE [cqm2023].[usp_SavePop1NumeratorCMS155v11_NQF0024]  
@RequestId BIGINT  
AS
BEGIN
...
FROM cqm2023.[DoctorCQMCalcPop1CMS155v11_NQF0024] IPP WITH(NOLOCK)
INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
INNER JOIN cqm2023.PatientPhysicalExamCodes bmi WITH(NOLOCK) ON bmi.PatientId = IPP.PatientId
INNER JOIN [cqm2023].[SysLookUpCMS155v11_NQF0024] cqm1 WITH(NOLOCK) ON cqm1.Code = bmi.Code
INNER JOIN cqm2023.PatientPhysicalExamCodes height WITH(NOLOCK) ON height.PatientId = IPP.PatientId
INNER JOIN [cqm2023].[SysLookUpCMS155v11_NQF0024] cqm2 WITH(NOLOCK) ON cqm2.Code = height.Code
INNER JOIN cqm2023.PatientPhysicalExamCodes weight WITH(NOLOCK) ON weight.PatientId = IPP.PatientId
INNER JOIN [cqm2023].[SysLookUpCMS155v11_NQF0024] cqm3 WITH(NOLOCK) ON cqm3.Code = weight.Code
