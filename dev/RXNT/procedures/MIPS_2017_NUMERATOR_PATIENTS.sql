...
-- =============================================
CREATE PROCEDURE [dbo].[MIPS_2017_NUMERATOR_PATIENTS] 
	@dr_id int, 
...
BEGIN
	...
	BEGIN
		...
		IF @MeasureCode = 'EPR'
		BEGIN
			with Numerator_Patient as 
			(
				  select distinct pa_id as NumerPatient 
		...
		IF @MeasureCode= 'MDR'
			BEGIN
				...
				left join patients p on p.pa_id=num.pa_id
		...
		IF @MeasureCode= 'HIE'	
			BEGIN
				...
				( 
					...
					--from  dbo.patient_encounters    pat  
					--inner join Patient_CCD_request_queue PCCD with(nolock) on PCCD.pa_id = pat.pa_id
					--inner join Patient_CCD_request_batch PCCB with(nolock) on PCCD.batchid = PCCB.batchid
					--where  1=1  
					--and   PCCB.active = 1
					--and   PCCD.created_date  between @reporting_start_date and @reporting_end_date 
					SELECT DISTINCT de.pat_id as pa_id FROM  dbo.direct_email_sent_messages de with(nolock) 
					INNER JOIN patient_encounters pe ON pe.patient_id=de.pat_id
