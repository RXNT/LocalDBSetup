...
Description			:	This procedure is used to insert First Name Matching Last Name
Last Modified By	:
Last Modifed Date	:
=======================================================================================
*/
CREATE PROCEDURE [rpt].[usp_InsertDeduplication_FnMatchingLn]
(
	@CompanyId BIGINT,
...
BEGIN
	...
	WHERE	PST.Code = 'FNMLN'
	

	-- First Transition table	
	SELECT  dG.dc_id, ISNULL(pat.pa_last, '') + '~~' + ISNULL(pat.pa_last, '') As DuplicationText, COUNT(*) As Cnt
	INTO	#TempPatients
	...
	Group by dG.dc_id,  ISNULL(pat.pa_last, '') + '~~' + ISNULL(pat.pa_last, '')
	having COUNT(*) > 1
	...
	FROM		dbo.patients pat WITH (NOLOCK)
				...
				INNER JOIN rpt.DoctorCompanyDeduplicationTransition tmp WITH (NOLOCK) ON tmp.CompanyId = dg.dc_id 
					...
					AND (ISNULL(pat.pa_first, '') + '~~' + ISNULL(pat.pa_first, '') = tmp.DuplicationText
					OR ISNULL(pat.pa_last, '') + '~~' + ISNULL(pat.pa_last, '') = tmp.DuplicationText)
	WHERE		dg.dc_id = @CompanyId
