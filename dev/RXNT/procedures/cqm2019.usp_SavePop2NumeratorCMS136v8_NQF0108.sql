...
CREATE PROCEDURE [cqm2019].[usp_SavePop2NumeratorCMS136v8_NQF0108]
	@RequestId BIGINT  
AS
BEGIN
	...
	(
		...
		LEFT JOIN cqm2019.DoctorCQMCalcPop2CMS136v8_NQF0108 IPP  WITH(NOLOCK) on IPP.PatientId = pmc.PatientId AND IPP.RequestId = @RequestId
	...
	SET Numerator = 1
	FROM cqm2019.DoctorCQMCalcPOP2CMS136v8_NQF0108 IPP WITH(NOLOCK)
	INNER JOIN cqm2019.PatientEncounterCodes pec  WITH(NOLOCK) on pec.PatientId = IPP.PatientId
	INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN [cqm2019].[SysLookupCMS136v8_NQF0108] cqm  WITH(NOLOCK) ON pec.Code = cqm.code	 
	INNER JOIN InitialADHDMedication M ON M.pa_id=IPP.PatientId	
	INNER JOIN cqm2019.PatientEncounterCodes pec1  WITH(NOLOCK) on pec1.PatientId = IPP.PatientId
