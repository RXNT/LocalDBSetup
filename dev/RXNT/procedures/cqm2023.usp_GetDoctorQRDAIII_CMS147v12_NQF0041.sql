...
CREATE   PROCEDURE [cqm2023].[usp_GetDoctorQRDAIII_CMS147v12_NQF0041]  
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
	 ...
	 BEGIN
		 ...
		 FROM  [cqm2023].[DoctorCQMCalcPop1CMS147v12_NQF0041] N WITH(NOLOCK)
		...
		INNER JOIN [cqm2023].[DoctorCQMCalcPop1CMS147v12_NQF0041] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		FROM patients pat WITH(NOLOCK) 
		INNER JOIN [cqm2023].[DoctorCQMCalcPop1CMS147v12_NQF0041] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		INNER JOIN  [cqm2023].SysLookupCMS147v12_NQF0041 cqm WITH(NOLOCK) ON pat.pa_sex = cqm.Code
		...
		INNER JOIN [cqm2023].[DoctorCQMCalcPop1CMS147v12_NQF0041] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
		WHERE N.RequestId = @RequestId 
