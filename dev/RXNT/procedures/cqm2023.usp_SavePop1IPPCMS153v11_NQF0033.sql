...
-- =============================================
CREATE    PROCEDURE [cqm2023].[usp_SavePop1IPPCMS153v11_NQF0033]
	@RequestId BIGINT  
AS
BEGIN
	...
	LEFT JOIN cqm2023.DoctorCQMCalcPop1CMS153v11_NQF0033 IPP WITH(NOLOCK) ON IPP.PatientId = pat.pa_id AND IPP.RequestId = @RequestId
	...
	INSERT INTO [cqm2023].[DoctorCQMCalcPop1CMS153v11_NQF0033]
	...
	SELECT DISTINCT TOP(@MaxRecords) i.pa_id, 1, 1, @DoctorId, @RequestId FROM (
		...
		INNER JOIN cqm2023.PatientDiagnosisCodes pdc  WITH(NOLOCK)ON pdc.PatientId = tmp.pa_id
		INNER JOIN patient_active_diagnosis pad with(NOLOCK) ON pad.pad = pdc.DiagnosisId
		INNER JOIN [cqm2023].[SysLookupCMS153v11_NQF0033] as cqm_pad with(NOLOCK) on pdc.Code = cqm_pad.Code
		...
		SELECT tmp.pa_id FROM #tempCMS153v11_NQF0033 tmp 
		INNER JOIN patients pat WITH(NOLOCK) ON tmp.pa_id=pat.pa_id
		INNER JOIN cqm2023.PatientMedicationCodes pmc WITH(NOLOCK) ON pmc.PatientId = tmp.pa_id
		INNER JOIN cqm2023.SysLookupCMS153v11_NQF0033 cqm WITH(NOLOCK) ON pmc.Code = cqm.code AND pmc.MedicationId IS NOT NULL
	...
	LEFT JOIN cqm2023.DoctorCQMCalcPop1CMS153v11_NQF0033 IPP WITH(NOLOCK) ON IPP.PatientId = i.pa_id AND IPP.RequestId = @RequestId
	...
	SELECT DISTINCT TOP(@MaxRecords) i.pa_id, 1, 1, @DoctorId, @RequestId FROM (
		SELECT tmp.pa_id FROM #tempCMS153v11_NQF0033 tmp 
		INNER JOIN patients pat WITH(NOLOCK) ON tmp.pa_id=pat.pa_id
		INNER JOIN cqm2023.PatientLaboratoryTestCodes pltc  WITH(NOLOCK)ON pltc.PatientId = tmp.pa_id
		...
		INNER JOIN patients pat WITH(NOLOCK) ON tmp.pa_id=pat.pa_id
		INNER JOIN cqm2023.PatientMedicationCodes pmc WITH(NOLOCK) ON pmc.PatientId = tmp.pa_id
		INNER JOIN cqm2023.SysLookupCMS153v11_NQF0033 cqm WITH(NOLOCK) ON pmc.Code = cqm.code AND pmc.PrescriptionId IS NOT NULL
		WHERE pmc.PerformedFromDate BETWEEN @StartDate AND @EndDate
		AND pmc.DoctorId = @DoctorId AND cqm.QDMCategoryId = 5 AND cqm.CodeSystemId = 26 AND cqm.ValueSetId IN (73)
		UNION
		SELECT tmp.pa_id FROM #tempCMS153v11_NQF0033 tmp 
		INNER JOIN patients pat WITH(NOLOCK) ON tmp.pa_id=pat.pa_id
		INNER JOIN cqm2023.PatientProcedureCodes ppc_s WITH(NOLOCK) ON ppc_s.PatientId = tmp.pa_id
		INNER JOIN cqm2023.SysLookupCMS153v11_NQF0033 cqm WITH(NOLOCK) on cqm.Code = ppc_s.Code
		...
		SELECT tmp.pa_id FROM #tempCMS153v11_NQF0033 tmp 
		INNER JOIN patients pat WITH(NOLOCK) ON tmp.pa_id=pat.pa_id
		INNER JOIN cqm2023.PatientDiagnosticStudyCodes pdsc WITH(NOLOCK) ON pdsc.PatientId = tmp.pa_id
