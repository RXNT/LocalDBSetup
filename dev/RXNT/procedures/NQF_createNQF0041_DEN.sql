...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[NQF_createNQF0041_DEN]
    @doctorid BIGINT , 
	@startdate DATETIME ,
	@enddate DATETIME 
AS
...
BEGIN TRY
	select count(distinct PAT.pa_id)SM from patients PAT inner join 
                     ...
                     and NQF.IsActive=1 and NQF.IsExclude=0 and dr_id=@doctorid AND PP.date_performed between 
                     @startdate AND @enddate 
                     group by pa_id having count(pa_id) > 2))PPC on PAT.pa_id=PPC.pa_id inner join patient_procedures PP on PAT.pa_id=PP.pa_id 
                     ...
                     @startdate AND @enddate AND PP.CODE in (select code from NQF_Codes where NQF_id='0041' and  code_type='CPT' 
