...
-- =============================================
CREATE PROCEDURE [cqm2018].[usp_SaveQDMCMS117v6_NQF0038_LabTest]
	@RequestId BIGINT
AS
BEGIN
	...
	INSERT INTO cqm2018.PatientLaboratoryTestCodes	
	...
	[cqm2018].[FindClosestEncounter](lrf.obs_date, lm.pat_id, @DoctorId) AS EncounterId,
	...
	INNER JOIN [cqm2018].[SysLookupCMS117v6_NQF0038] cqm WITH(NOLOCK) ON lrf.obs_bat_ident = cqm.code 
	LEFT JOIN [cqm2018].[PatientLaboratoryTestCodes] pltc WITH(NOLOCK) on lrf.lab_result_info_id = pltc.LaboratoryTestId AND pltc.EncounterId = EncounterId 
	WHERE lm.dr_id = @DoctorId
	AND lrf.obs_date between @StartDate AND @EndDate
	AND cqm.QDMCategoryId = 9 AND cqm.CodeSystemId = 9 
