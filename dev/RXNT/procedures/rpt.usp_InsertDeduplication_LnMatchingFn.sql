...
Description			:	This procedure is used to insert Last Name Matching First Name
Last Modified By	:
Last Modifed Date	:
=======================================================================================
*/
CREATE PROCEDURE [rpt].[usp_InsertDeduplication_LnMatchingFn]
(
	@CompanyId BIGINT,
...
BEGIN
	...
	WHERE	PST.Code = 'LNMFN'

	-- First Transition table	
	SELECT  dG.dc_id, ISNULL(pat.pa_first, '') + '~~' + ISNULL(pat.pa_first, '') As DuplicationText, COUNT(*) As Cnt
	INTO	#TempPatients
	...
	Group by dG.dc_id,  ISNULL(pat.pa_first, '') + '~~' + ISNULL(pat.pa_first, '')
	having COUNT(*) > 1
	...
	FROM		dbo.patients pat WITH (NOLOCK)
				...
				INNER JOIN rpt.DoctorCompanyDeduplicationTransition tmp WITH (NOLOCK) ON tmp.CompanyId = dg.dc_id 
					...
					AND (ISNULL(pat.pa_last, '') + '~~' + ISNULL(pat.pa_last, '') = tmp.DuplicationText
					OR ISNULL(pat.pa_first, '') + '~~' + ISNULL(pat.pa_first, '') = tmp.DuplicationText)
	WHERE		dg.dc_id = @CompanyId
