...
-- =============================================
CREATE PROCEDURE [dbo].[CQM_CMS155v2_NQF0024_POP1_NUMER1] 
...
BEGIN
	...
	BEGIN
         ...
         INNER JOIN patient_vitals pvt ON pvt.pa_id=pp.pa_id where cqm.version=2 AND cqm.NQF_id='0024' and cqm.code_type IN ('CPT','SNOMEDCT')
         ...
         AND pvt.record_date between @fromdate AND @todate
         AND pvt.pa_wt>0 AND pvt.pa_ht>0 AND pvt.pa_bmi>0
	...
	BEGIN
        ...
        datediff(month,PAT.pa_dob,CONVERT(datetime,@fromdate,101)) BETWEEN 3*12 AND 17*12 GROUP BY medicare_type_code
