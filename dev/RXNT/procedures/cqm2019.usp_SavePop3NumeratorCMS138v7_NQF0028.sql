...
-- =============================================
CREATE PROCEDURE [cqm2019].[usp_SavePop3NumeratorCMS138v7_NQF0028]
	@RequestId BIGINT 
AS
BEGIN
	...
	FROM cqm2019.DoctorCQMCalcPop3CMS138v7_NQF0028 IPP WITH(NOLOCK)
	...
	AND EXISTS(SELECT TOP 1 * FROM  cqm2019.SysLookupCMS138v7_NQF0028 cqm_r WITH(NOLOCK) WHERE cqm_r.QDMCategoryId = 17 AND cqm_r.CodeSystemId = 13 AND cqm_r.ValueSetId = 167 AND cqm_r.Code = prc.Code)
	...
	INNER JOIN patients pat WITH(NOLOCK) ON pat.pa_id=IPP.PatientId
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN cqm2019.PatientRiskCategoryOrAssessmentCodes  prc WITH(NOLOCK) ON IPP.PatientId=prc.PatientId
	...
	AND pfd.flag_id IN (6,7) AND EXISTS(SELECT * FROM [cqm2019].[SysLookupCMS138v7_NQF0028] cqm WITH(NOLOCK) WHERE cqm.ValueSetId=167 AND prc.Code = cqm.code )
