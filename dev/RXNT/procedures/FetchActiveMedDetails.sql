...
-- =============================================
CREATE PROCEDURE [dbo].[FetchActiveMedDetails]	
...
IF @bsingledosage = 1
BEGIN
    SELECT DISTINCT  0 ETC_ID, '' ETC_NAME, R.MED_MEDID_DESC, R.MEDID, PAM.DATE_ADDED, DR.DR_FIRST_NAME AD_FNAME, DR.DR_LAST_NAME AD_LNAME, S.* FROM PATIENT_ACTIVE_MEDS PAM WITH (NOLOCK) INNER JOIN RMIID1 R WITH (NOLOCK) ON PAM.DRUG_ID = R.MEDID INNER JOIN DOCTORS DR WITH (NOLOCK) ON PAM.ADDED_BY_DR_ID = DR.DR_ID LEFT OUTER JOIN 
(
...
SELECT MAX(P.PRES_ID) PRESIDS FROM PRESCRIPTIONS P WITH (NOLOCK) INNER JOIN PATIENTS PT ON P.PA_ID =
...
AND PD.DDID NOT IN (SELECT DISTINCT DDID FROM PRESCRIPTIONS P WITH(NOLOCK) INNER JOIN PATIENTS PT WITH(NOLOCK) ON P.PA_ID =
PT.PA_ID INNER JOIN PRESCRIPTION_DETAILS PD WITH(NOLOCK) ON P.PRES_ID = PD.PRES_ID WHERE PT.PA_ID = @PA_ID  AND P.PRES_VOID = 0 AND P.PRES_APPROVED_DATE IS NOT NULL AND HISTORY_ENABLED = 1 AND P.PRES_APPROVED_DATE > DATEADD(MM, -@num_months ,GETDATE()))
