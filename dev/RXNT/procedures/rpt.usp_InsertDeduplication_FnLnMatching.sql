...
Description			:	This procedure is used to insert First Name Last Name Matching
Last Modified By	:
Last Modifed Date	:
=======================================================================================
*/
CREATE PROCEDURE [rpt].[usp_InsertDeduplication_FnLnMatching]
(
	@CompanyId BIGINT,
...
BEGIN
	...
	WHERE	PST.Code = 'FNLNM'

	-- First Transition table	
	Select		dg.dc_id, ISNULL(pat.pa_first, '')  + '~~' + ISNULL(pat.pa_last, '') As DuplicationText
	INTO		#TempPatients
	...
	FROM		dbo.patients pat WITH (NOLOCK)
				...
				INNER JOIN rpt.DoctorCompanyDeduplicationTransition tmp WITH (NOLOCK) ON tmp.CompanyId = dg.dc_id 
					...
					AND ISNULL(pat.pa_first, '') + '~~' +  ISNULL(pat.pa_last, '') = tmp.DuplicationText
	WHERE		dg.dc_id = @CompanyId
