...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[NQF_createNQF0421_POP2_EXCLUSION]
...
BEGIN TRY
   declare @query varchar(max);
	set @query ='select count(distinct PAT.pa_id)SM from patients PAT inner join ((select pa_id from patient_active_diagnosis 
                     where pa_id in ('+@PatientIds+') and icd9 in (select code from NQF_codes NQF 
                     where NQF_id=0421 and NQF.code_type=''ICD-9'' and criteriatype=''EXCL'' and criteria=1 AND IsActive=1)) UNION 
