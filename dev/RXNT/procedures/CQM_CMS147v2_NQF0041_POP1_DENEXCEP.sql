...
-- =============================================
CREATE PROCEDURE [dbo].[CQM_CMS147v2_NQF0041_POP1_DENEXCEP] 
...
BEGIN
	...
	BEGIN
          ...
          (
			(select pa_id from patient_procedures PP 
				...
				and cqm.code_type='CPT' and criteriatype='POP1' and criteria=1
          ...
          where datediff(month,PAT.pa_dob,CONVERT(datetime,@fromdate,101)) >= 6 and
           ...
           AND PAA.allergy_type=1 and PAA.allergy_id in
          (SELECT CODE FROM CQM_Codes 
			where version=2 AND IsActive=1 AND IsExclude=0 
			AND NQF_id='0041' AND criteriatype='NUM1' 
			AND code_type='CVX'
		   ) 
		   AND PP.date_performed between @fromdate AND @todate 
		   AND PP.CODE in 
		   (select code from CQM_Codes 
			where version=2 AND NQF_id='0041' and  code_type='CPT'
	...
	BEGIN
		select pa_race_type ,COUNT(DISTINCT pat.pa_id) SM from patients PAT inner join
          ...
          @fromdate AND @todate))
          group by pa_id having count(pa_id) > 0) union (select pa_id from patient_procedures PP INNER JOIN CQM_Codes cqm on PP.code=cqm.code
          ...
          (SELECT CODE FROM CQM_Codes where version=2 AND IsActive=1 AND IsExclude=0 AND NQF_id='0041' AND criteriatype='NUM1' AND
          code_type='CVX') AND PP.date_performed between
          @fromdate AND @todate AND PP.CODE
          in (select code from CQM_Codes where version=2 AND NQF_id='0041' and  code_type='CPT'
          and IsActive = 1 and criteria IN (2,3,4) and criteriatype = 'Common1')
