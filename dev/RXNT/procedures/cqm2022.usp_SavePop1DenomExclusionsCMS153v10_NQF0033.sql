...
-- =============================================
CREATE   PROCEDURE [cqm2022].[usp_SavePop1DenomExclusionsCMS153v10_NQF0033]
	@RequestId BIGINT 
AS
BEGIN
	...
	FROM [cqm2022].[DoctorCQMCalcPop1CMS153v10_NQF0033] IPP WITH(NOLOCK)
	...
	AND EXISTS (
		SELECT 1 FROM cqm2022.PatientDiagnosticStudyCodes pdsc  WITH(NOLOCK)
	...
	AND pltc.DoctorId = @DoctorId AND pltc.PerformedFromDate BETWEEN @StartDate AND @EndDate
	AND EXISTS (
		SELECT 1 FROM cqm2022.PatientMedicationCodes pmc 
		INNER JOIN patients pat1 WITH(NOLOCK) ON pmc.PatientId=pat1.pa_id
		INNER JOIN [cqm2022].[SysLookupCMS153v10_NQF0033] as cqm with(NOLOCK) on pmc.Code = cqm.Code
