...
Description			:	This procedure is used to insert Last Name DOB Matching
Last Modified By	:
Last Modifed Date	:
=======================================================================================
*/
CREATE PROCEDURE [rpt].[usp_InsertDeduplication_LnDOBMatching]
(
	@CompanyId BIGINT,
...
BEGIN
	...
	WHERE	PST.Code = 'LNDOB'

	-- First Transition table	
	Select		dg.dc_id, ISNULL(pat.pa_last, '') + '~~' + 
	...
	FROM		dbo.patients pat WITH (NOLOCK)
				...
				INNER JOIN rpt.DoctorCompanyDeduplicationTransition tmp WITH (NOLOCK) ON tmp.CompanyId = dg.dc_id 
																						...
																						AND ISNULL(pat.pa_last, '') + '~~' + CONVERT(VARCHAR(50), CASE WHEN pat.pa_dob is null then '1901-01-01 00:00:00' ELSE pat.pa_dob END, 101) = tmp.DuplicationText
	WHERE		dg.dc_id = @CompanyId
