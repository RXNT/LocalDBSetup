...
-- =============================================
CREATE   PROCEDURE [cqm2022].[usp_SavePop1NumeratorCMS138v11_NQF0028]
	@RequestId BIGINT 
AS
BEGIN
	...
	FROM [cqm2022].[DoctorCQMCalcPop1CMS138v11_NQF0028] IPP WITH(NOLOCK)
...
--	INNER JOIN [cqm2022].[SysLookupCMS138v11_NQF0028] cqm_r WITH(NOLOCK) on cqm_r.Code = prc.Code
	...
	AND prc.PerformedFromDate BETWEEN @measureStartDate AND @EndDate AND EXISTS(SELECT TOP 1* FROM  [cqm2022].[SysLookupCMS138v11_NQF0028] cqm_r WITH(NOLOCK) WHERE cqm_r.QDMCategoryId = 17 AND cqm_r.CodeSystemId = 25 AND cqm_r.ValueSetId = 167 AND cqm_r.Code = prc.Code)
