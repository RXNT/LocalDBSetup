...
-- =============================================
CREATE    PROCEDURE [cqm2023].[usp_SaveQDMCMS136v12_NQF0108_Diagnosis]
	@RequestId BIGINT
AS
BEGIN
	...
	INNER JOIN cqm2023.SysLookupCMS136v12_NQF0108 cqm  WITH(NOLOCK) ON pad.icd9 = cqm.code 
	...
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (2,21,23,27,28,29,30,31) AND cqm.ValueSetId IN (35)
	...
	FROM patient_active_diagnosis pad WITH(NOLOCK)
	INNER JOIN patients pat WITH(NOLOCK) ON pad.pa_id=pat.pa_id
	INNER JOIN cqm2023.SysLookupCMS136v12_NQF0108 cqm  WITH(NOLOCK) ON pad.icd10 = cqm.code
	LEFT JOIN cqm2023.PatientDiagnosisCodes pdc  WITH(NOLOCK) on pad.pad = pdc.DiagnosisId AND pdc.EncounterId = EncounterId AND pdc.Code = cqm.Code
	WHERE pad.added_by_dr_id = @DoctorId
	AND pad.date_added between @StartDate AND @EndDate
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (2,21,23,27,28,29,30,31) AND cqm.ValueSetId IN (35)
	...
	FROM patient_active_diagnosis pad  WITH(NOLOCK)
	INNER JOIN patients pat WITH(NOLOCK) ON pad.pa_id=pat.pa_id
	INNER JOIN cqm2023.SysLookupCMS136v12_NQF0108 cqm  WITH(NOLOCK) ON pad.snomed_code = cqm.code
	LEFT JOIN cqm2023.PatientDiagnosisCodes pdc  WITH(NOLOCK) on pad.pad = pdc.DiagnosisId AND pdc.EncounterId = EncounterId AND pdc.Code = cqm.Code
	WHERE pad.added_by_dr_id = @DoctorId
	AND pad.date_added between @StartDate AND @EndDate
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (2,21,23,27,28,29,30,31) AND cqm.ValueSetId IN (35)
