...
-- =============================================
CREATE PROCEDURE [dbo].[CQM_CMS136v3_NQF0108_POP1_IPP] 
...
BEGIN
	...
	BEGIN
		...
		(	SELECT pres_ipp.pa_id 
			FROM prescriptions pres_ipp  
			INNER JOIN prescription_details pd_ipp ON pres_ipp.pres_id=pd_ipp.pres_id 
			INNER JOIN revdel0 r1 WITH(NOLOCK) 
			ON CAST(pd_ipp.ddid AS VARCHAR)=r1.evd_fdb_vocab_id   
			INNER JOIN REVDVT0 R2 WITH(NOLOCK) 
			ON  r1.evd_ext_vocab_type_id=r2.evd_vocab_type_id    
			...
			WHERE PP_ipp.dr_id=@doctorid 
			AND PP_ipp.date_performed between @fromdate AND @todate
			AND cqm_pam_ipp.version=2 AND cqm_pam_ipp.nqf_id='0108' 
			AND cqm_pam_ipp.isactive=1 AND cqm_pam_ipp.code_type ='RXNORM'  
			AND cqm_pam_ipp.criteriatype='COMMON1' 
			AND cqm_pam_ipp.isexclude=0 AND cqm_pam_ipp.criteria=1
		...
		(   SELECT pres_ipp.pa_id 
			...
			INNER JOIN cqm_codes cqm_pam_ipp 
			ON CAST(R1.EVD_EXT_VOCAB_ID AS VARCHAR)= cqm_pam_ipp.code  
			INNER JOIN 	patient_procedures PP_ipp 
			ON PP_ipp.pa_id=pres_ipp.pa_id  
			AND DATEDIFF(day,pres_ipp.pres_approved_date,PP_ipp.date_performed) 
			BETWEEN 0 AND 30  
			INNER JOIN cqm_codes cqm_pp_ipp ON cqm_pp_ipp.code = PP_ipp.code  
	...
	BEGIN
        ...
        AND DATEDIFF(day,pres_ipp.pres_approved_date,PP_ipp.date_performed) 
        BETWEEN 0 AND 30   
        INNER JOIN cqm_codes cqm_pp_ipp 
        ON cqm_pp_ipp.code = PP_ipp.code   
         WHERE PP_ipp.dr_id= @doctorid 
         AND PP_ipp.date_performed between @fromdate AND @todate
         AND cqm_pam_ipp.version=2 AND cqm_pam_ipp.nqf_id='0108' 
         AND cqm_pam_ipp.isactive=1 AND cqm_pam_ipp.code_type ='RXNORM'  
