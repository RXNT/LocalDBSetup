...
-- Description: Load the transmit epcs prescription fro declined

-- =============================================  
CREATE  PROCEDURE [dbo].[SS_LoadTransmitEPCSPrescriptionForDeclined]
 @presid int,  
 @pdid int  
AS  
  ...
  BEGIN
	...
	SET NOCOUNT ON;
		...
		@pbm_id pbm_id,@ic_group_numb [ic_group_numb],@pa_bin pa_bin,@pbm_id pbm_id,@card_holder_last  card_holder_last,@card_holder_mi card_holder_mi,@card_holder_first card_holder_first,@card_holder_id card_holder_id,
		...
		FROM PRESCRIPTIONS P WITH(NOLOCK) 
		INNER JOIN PRESCRIPTION_DETAILS PD WITH(NOLOCK) ON P.PRES_ID = PD.PRES_ID 
		LEFT OUTER JOIN scheduled_rx_archive SRA WITH(NOLOCK) ON P.PRES_ID = SRA.PRES_ID  AND PD.pd_id = SRA.pd_id
