...
-- =============================================
CREATE     PROCEDURE [cqm2023].[usp_GetCQMMeasureInfo] 
	@NQFNumber VARCHAR(MAX) 
AS
BEGIN

	DECLARE @MeasureId BIGINT = 0
	SELECT @MeasureId = MeasureInfoId FROM cqm2023.SysLookupCqmMeasuresInfo WITH(NOLOCK) WHERE NQFNumber = @NQFNumber
	
	SELECT MeasureInfoId, MeasureTitle, ReferenceId, Description, MeasureNumber, NQFNumber, SetID
	FROM cqm2023.SysLookupCqmMeasuresInfo WITH(NOLOCK)
	...
	FROM cqm2023.SysLookupCqmMeasurePopulationInfo WITH(NOLOCK)
	WHERE MeasureInfoId = @MeasureId
	...
	FROM cqm2023.SysLookupCqmMeasureStratumInfo WITH(NOLOCK)
	WHERE MeasureInfoId = @MeasureId 
