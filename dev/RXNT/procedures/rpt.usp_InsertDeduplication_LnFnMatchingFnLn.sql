...
Description			:	This procedure is used to insert First Name Last Name Matching Last Name First Name
Last Modified By	:
Last Modifed Date	:
=======================================================================================
*/
CREATE PROCEDURE [rpt].[usp_InsertDeduplication_LnFnMatchingFnLn]
(
	@CompanyId BIGINT,
...
BEGIN
	...
	WHERE	PST.Code = 'LFMFL'

	-- First Transition table	
	SELECT  dG.dc_id, ISNULL(pat.pa_last, '') + '~~' + ISNULL(pat.pa_first, '') + '~~' +
						ISNULL(pat.pa_first, '') + '~~' + ISNULL(pat.pa_last, '') As DuplicationText, COUNT(*) As Cnt
	INTO	#TempPatients
	FROM	dbo.patients pat WITH (NOLOCK)
			INNER JOIN dbo.doc_groups DG WITH (NOLOCK) ON DG.dg_id = pat.dg_id 
			INNER JOIN dbo.doc_groups DG1 WITH (NOLOCK) on DG1.dc_id = DG.dc_id
			INNER JOIN dbo.patients pat1 WITH (NOLOCK) ON pat1.dg_id = dg1.dg_id 
															AND pat1.pa_first = pat.pa_last 
															AND pat1.pa_last = pat.pa_first
	...
	Group by dG.dc_id,  ISNULL(pat.pa_last, '') + '~~' + ISNULL(pat.pa_first, '')+ '~~' +
	...
	FROM		dbo.patients pat WITH (NOLOCK)
				...
				INNER JOIN rpt.DoctorCompanyDeduplicationTransition tmp WITH (NOLOCK) ON tmp.CompanyId = dg.dc_id 
					...
					AND ((ISNULL(pat.pa_last, '') + '~~' + ISNULL(pat.pa_first, '') + '~~' +
						ISNULL(pat.pa_first, '') + '~~' + ISNULL(pat.pa_last, '')  = tmp.DuplicationText)
						OR (ISNULL(pat.pa_first , '') + '~~' + ISNULL(pat.pa_last, '') + '~~' +
						ISNULL(pat.pa_last, '') + '~~' + ISNULL(pat.pa_first, '')  = tmp.DuplicationText))
	WHERE		dg.dc_id = @CompanyId
