...
Description			:	This procedure is used to get state wise total eRx Count based on input
Last Modified By	:
Last Modifed Date	:
=======================================================================================
*/
CREATE PROCEDURE [support].[usp_GetStateWiseTotaleRxCount]
	...
	@StateCode					VARCHAR(5)=NULL -- To get count for specific State
AS
BEGIN
	...
	SELECT		COUNT(1) as eRxCount, ST.state_code AS StateCode, ST.state AS State
	FROM		prescriptions P WITH(NOLOCK)
				...
				INNER JOIN doctors DR WITH(NOLOCK) ON P.prim_dr_id=DR.dr_id
				INNER JOIN State ST WITH(NOLOCK) ON DR.dr_state=ST.state_code
	WHERE		P.pres_void = 0 AND
				...
				(
					@StateCode IS NULL OR
					(
						@StateCode IS NOT NULL AND
						ST.state_code = @StateCode
					)
				)
GROUP BY		ST.state_code, ST.state
