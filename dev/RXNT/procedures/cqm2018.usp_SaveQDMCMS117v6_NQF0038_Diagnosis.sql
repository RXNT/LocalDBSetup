...
-- =============================================
CREATE PROCEDURE [cqm2018].[usp_SaveQDMCMS117v6_NQF0038_Diagnosis]
	@RequestId BIGINT
AS
BEGIN
	...
	INNER JOIN [cqm2018].[SysLookupCMS117v6_NQF0038] cqm WITH(NOLOCK) ON pad.icd9 = cqm.code 
	...
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (15,8,11,13,14) 
	AND cqm.ValueSetId IN (79,99,100,101,102,103,104,105,106,107,108,109,110,113,116,119,122,129,130,131,136,146,149,150)
	...
	pad.pa_id, pad.added_by_dr_id,cqm.CodeSystemId, cqm.Code, pad.date_added
	FROM patient_active_diagnosis pad WITH(NOLOCK)
	INNER JOIN [cqm2018].[SysLookupCMS117v6_NQF0038] cqm WITH(NOLOCK) ON pad.icd10 = cqm.code 
	LEFT JOIN [cqm2018].[PatientDiagnosisCodes] pdc  WITH(NOLOCK) on pad.pad = pdc.DiagnosisId AND pdc.EncounterId = EncounterId 
	WHERE pad.added_by_dr_id = @DoctorId
	AND pad.date_added between @StartDate AND @EndDate
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (15,8,11,13,14) 
	...
	pad.pa_id, pad.added_by_dr_id,cqm.CodeSystemId, cqm.Code, pad.date_added
	FROM patient_active_diagnosis pad WITH(NOLOCK)
	INNER JOIN [cqm2018].[SysLookupCMS117v6_NQF0038] cqm WITH(NOLOCK) ON pad.snomed_code = cqm.code 
	LEFT JOIN [cqm2018].[PatientDiagnosisCodes] pdc WITH(NOLOCK) on pad.pad = pdc.DiagnosisId AND pdc.EncounterId = EncounterId 
	WHERE pad.added_by_dr_id = @DoctorId
	AND pad.date_added between @StartDate AND @EndDate
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (15,8,11,13,14) 
