...
-- =============================================
CREATE PROCEDURE [cqm2018].[usp_SaveQDMCMS153v6_NQF0033_Diagnosis]
	@RequestId BIGINT
AS
BEGIN
	...
	INNER JOIN cqm2018.SysLookUpCMS153v6_NQF0033 cqm WITH(NOLOCK) ON pad.icd9 = cqm.code 
	...
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (13,5,11,14,8) 
	AND cqm.ValueSetId IN (70,72,77,78,79,80,84,90)
	...
	pad.pa_id, pad.added_by_dr_id,cqm.CodeSystemId, cqm.Code, pad.date_added
	FROM patient_active_diagnosis pad WITH(NOLOCK)
	INNER JOIN cqm2018.SysLookUpCMS153v6_NQF0033 cqm  WITH(NOLOCK)ON pad.icd10 = cqm.code 
	LEFT JOIN cqm2018.PatientDiagnosisCodes pdc WITH(NOLOCK) on pad.pad = pdc.DiagnosisId AND pdc.EncounterId = EncounterId 
	WHERE pad.added_by_dr_id = @DoctorId
	AND (pad.date_added BETWEEN @StartDate AND @EndDate OR pad.status_date BETWEEN @StartDate AND @EndDate) 
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (13,5,11,14,8) 
	...
	pad.pa_id, pad.added_by_dr_id,cqm.CodeSystemId, cqm.Code, pad.date_added
	FROM patient_active_diagnosis pad WITH(NOLOCK)
	INNER JOIN cqm2018.SysLookUpCMS153v6_NQF0033 cqm WITH(NOLOCK) ON pad.snomed_code = cqm.code 
	LEFT JOIN cqm2018.PatientDiagnosisCodes pdc WITH(NOLOCK) on pad.pad = pdc.DiagnosisId AND pdc.EncounterId = EncounterId 
	WHERE pad.added_by_dr_id = @DoctorId
	AND (pad.date_added BETWEEN @StartDate AND @EndDate OR pad.status_date BETWEEN @StartDate AND @EndDate) 
	AND cqm.QDMCategoryId = 1 AND cqm.CodeSystemId IN (13,5,11,14,8) 
