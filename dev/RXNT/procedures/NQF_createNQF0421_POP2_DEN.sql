...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[NQF_createNQF0421_POP2_DEN]
    @doctorid BIGINT,
    @startdate datetime,
    @enddate datetime
AS
...
BEGIN TRY
   select distinct PAT.pa_id from patients PAT inner join 
                    ...
                    where datediff(month,PAT.pa_dob,CONVERT(datetime,@startdate,101)) >= 18*12 and
                    datediff(month,PAT.pa_dob,CONVERT(datetime,@startdate,101)) <= 64*12 and
