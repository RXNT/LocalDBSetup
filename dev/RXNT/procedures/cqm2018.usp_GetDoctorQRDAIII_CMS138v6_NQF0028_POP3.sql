...
CREATE PROCEDURE [cqm2018].[usp_GetDoctorQRDAIII_CMS138v6_NQF0028_POP3]
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
	 ...
	 BEGIN
		 ...
		 FROM  [cqm2018].[DoctorCQMCalcPop3CMS138v6_NQF0028] N WITH(NOLOCK)
		 WHERE N.RequestId = @RequestId 
		 
		 --Race Wise
		 
		SELECT vwRace.RaceId AS Code,vwRace.Description AS Description,
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop3CMS138v6_NQF0028] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop3CMS138v6_NQF0028] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
