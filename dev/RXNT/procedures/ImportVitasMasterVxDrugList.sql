...
-- =============================================
CREATE PROCEDURE [dbo].[ImportVitasMasterVxDrugList]
	@dg_id int
AS
BEGIN
	...
	If exists(select top 1 * from RxNTReportUtils..VitasMasterList)
	Begin
		PRINT 'STARTED'
		create table #temp(GCN varchar(10))
		insert into #temp
		SELECT gcn from RxNTReportUtils..VitasMasterList
		...
		BEGIN
			...
			SELECT @DRUGNAME=medication from RxNTReportUtils..VitasMasterList 
			where gcn = @GCN
