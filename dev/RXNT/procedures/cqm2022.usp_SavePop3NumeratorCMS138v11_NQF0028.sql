...
-- =============================================
CREATE   PROCEDURE [cqm2022].[usp_SavePop3NumeratorCMS138v11_NQF0028]
	@RequestId BIGINT 
AS
BEGIN
	...
	FROM cqm2022.DoctorCQMCalcPop3CMS138v11_NQF0028 IPP WITH(NOLOCK)
	...
	--INNER JOIN cqm2022.SysLookupCMS138v11_NQF0028 cqm_r WITH(NOLOCK) on cqm_r.Code = prc.Code
	...
	AND EXISTS(SELECT TOP 1 * FROM  cqm2022.SysLookupCMS138v11_NQF0028 cqm_r WITH(NOLOCK) WHERE cqm_r.QDMCategoryId = 17 AND cqm_r.CodeSystemId = 25 AND cqm_r.ValueSetId = 167 AND cqm_r.Code = prc.Code)
	...
	AND pfd.flag_id IN (6,7) AND EXISTS(SELECT * FROM [cqm2022].[SysLookupCMS138v11_NQF0028] cqm WITH(NOLOCK) WHERE cqm.ValueSetId=167 AND prc.Code = cqm.code )
	...
	INNER JOIN  (
		...
		INNER JOIN [cqm2022].[SysLookupCMS138v11_NQF0028] cqm WITH(NOLOCK) ON pmc.Code = cqm.code AND pmc.MedicationId IS NOT NULL
		...
		INNER JOIN patients pat1 WITH(NOLOCK) ON pat.PatientId=pat1.pa_id
		INNER JOIN [cqm2022].PatientMedicationCodes pmc WITH(NOLOCK) ON pmc.PatientId = pat.PatientId
		INNER JOIN [cqm2022].[SysLookupCMS138v11_NQF0028] cqm WITH(NOLOCK) ON pmc.Code = cqm.code AND pmc.PrescriptionId IS NOT NULL
		WHERE pmc.PerformedFromDate BETWEEN pat.PerformedFromDate AND @EndDate
		AND pmc.DoctorId = @DoctorId AND QDMCategoryId = 5 AND cqm.CodeSystemId = 25 AND cqm.ValueSetId = 166
		UNION
		SELECT DISTINCT pat.PatientId from #tempCMS138v11_NQF0028  pat
		...
		INNER JOIN [cqm2022].[SysLookupCMS138v11_NQF0028] cqm WITH(NOLOCK) ON pic.Code = cqm.code 
