...
*/
CREATE PROCEDURE  [support].[AMABasedProcedureReportMonthly]
(
	@StartDate DATETIME,
	@EndDate	DATETIME
)
 ...
 (
	(SELECT COUNT(DISTINCT dr_id) FROM 
		(
			...
			WHERE pp.dr_id=dr.dr_id AND pp.record_modified_date BETWEEN @StartDate AND @EndDate AND pp.record_modified_date BETWEEN M.Dates AND DATEADD(MONTH,1,M.Dates))
			UNION
