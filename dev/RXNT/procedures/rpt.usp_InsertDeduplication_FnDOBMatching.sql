...
Description			:	This procedure is used to insert First Name DOB Matching
Last Modified By	:
Last Modifed Date	:
=======================================================================================
*/
CREATE PROCEDURE [rpt].[usp_InsertDeduplication_FnDOBMatching]
(
	@CompanyId BIGINT,
...
BEGIN
	...
	WHERE	PST.Code = 'FNDOB'	

	-- First Transition table	
	Select		dg.dc_id, ISNULL(pat.pa_first, '') + '~~' + 
	...
	FROM		dbo.patients pat WITH (NOLOCK)
				...
				INNER JOIN rpt.DoctorCompanyDeduplicationTransition tmp WITH (NOLOCK) ON tmp.CompanyId = dg.dc_id 
																						...
																						AND ISNULL(pat.pa_first, '') + '~~' + CONVERT(VARCHAR(50), CASE WHEN pat.pa_dob is null then '1901-01-01 00:00:00' ELSE pat.pa_dob END, 101) = tmp.DuplicationText
	WHERE		dg.dc_id = @CompanyId
