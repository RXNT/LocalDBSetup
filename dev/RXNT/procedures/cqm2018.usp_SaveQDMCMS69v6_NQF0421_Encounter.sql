...
CREATE PROCEDURE [cqm2018].[usp_SaveQDMCMS69v6_NQF0421_Encounter]
	@RequestId BIGINT
AS
BEGIN
	...
	@DoctorId as 'DoctorId', pv.pa_vt_id, @DefaultEncCode as 'Code', 3 as 'CodeSystem', pv.record_date
	FROM patient_vitals pv WITH(NOLOCK)
	LEFT JOIN cqm2018.PatientEncounterCodes pec WITH(NOLOCK) ON pec.PatientId = pv.pa_id AND pec.EncounterId = EncounterId AND pec.Code = @DefaultEncCode
