...
-- =============================================
-- Author:		Niyaz
-- Create date: 01-Feb-2018
-- Description:	To get the Doctor CQM request status 
-- =============================================

CREATE PROCEDURE [cqm2018].[usp_GetDoctorQRDAIII_CMS138v6_NQF0028_POP2]
	@DoctorId BIGINT,	
	@StartDate Date,
	@EndDate Date	
AS
BEGIN
	 ...
	 BEGIN
		 ...
		 FROM  [cqm2018].[DoctorCQMCalcPop2CMS138v6_NQF0028] N WITH(NOLOCK)
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop2CMS138v6_NQF0028] N WITH(NOLOCK) ON pat.pa_id = N.PatientId
		...
		SUM(CASE WHEN N.IPP = 1 THEN 1 ELSE 0 END) as IPP ,pyr.medicare_type_code AS Description,
		SUM(CASE WHEN N.Denominator = 1 THEN 1 ELSE 0 END) as Denominator, 
		SUM(CASE WHEN N.DenomExclusions = 1 THEN 1 ELSE 0 END) as DenomExclusions,
		SUM(CASE WHEN N.Numerator = 1 THEN 1 ELSE 0 END) as Numerator,
		SUM(CASE WHEN N.NumerExclusions = 1 THEN 1 ELSE 0 END) as NumerExclusions,
		...
		INNER JOIN [cqm2018].[DoctorCQMCalcPop2CMS138v6_NQF0028] N WITH(NOLOCK) ON pyr.pa_id = N.PatientId
