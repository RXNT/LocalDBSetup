...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[NQF_createNQF0001_NUMER]
    @doctorid BIGINT , 
	@startdate DATETIME ,
	@enddate DATETIME 
AS
...
BEGIN TRY
	select count(distinct PAT.pa_id)SM from patients PAT inner join 
                     ...
                     INNER JOIN patient_procedures PP on PP.pa_id=PAT.pa_id where datediff(month,PAT.pa_dob,CONVERT(datetime,@startdate,101)) >= 5*12 and
                     datediff(month,PAT.pa_dob,CONVERT(datetime,@startdate,101)) <= 40*12 AND not (pa_dob like '1901-01-01')  
                     AND PAD.ICD9 IN (select code from NQF_CODES where IsActive=1 AND IsExclude=0 AND NQF_id='0001' AND criteriatype='POP1' AND code_type='icd-9' ) 
                     AND PP.dr_id=@doctorid AND PAD.enabled=1 and PAD.type=1 and PP.date_performed between 
                     @startdate AND @enddate AND PP.code in (
                     select code from NQF_CODES where IsActive=1 AND IsExclude=0 AND NQF_id='0001' AND criteriatype='NUM1' AND code_type='CPT')
END TRY
	  BEGIN CATCH
