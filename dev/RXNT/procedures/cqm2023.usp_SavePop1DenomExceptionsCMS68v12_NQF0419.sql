...
-- =============================================
CREATE    PROCEDURE [cqm2023].[usp_SavePop1DenomExceptionsCMS68v12_NQF0419]
	@RequestId BIGINT  
AS
BEGIN
	...
	INNER JOIN cqm2023.PatientProcedureCodes ppc  WITH(NOLOCK) ON ppc.PatientId =  pat.pa_id
	LEFT JOIN cqm2023.DoctorCQMCalcPop1CMS68v12_NQF0419 IPP WITH(NOLOCK) ON IPP.PatientId = pat.pa_id AND IPP.RequestId = @RequestId AND CAST(ppc.PerformedFromDate AS DATE)=CAST(IPP.DatePerformed AS DATE) AND IPP.DenomExceptions =1
	...
	FROM patients pat WITH(NOLOCK) 
	INNER JOIN doc_groups dg WITH(NOLOCK) ON pat.dg_id=dg.dg_id
	INNER JOIN cqm2023.[PatientInterventionCodes] ppc  WITH(NOLOCK) ON ppc.PatientId =  pat.pa_id
	LEFT JOIN cqm2023.DoctorCQMCalcPop1CMS68v12_NQF0419 IPP WITH(NOLOCK) ON IPP.PatientId = pat.pa_id AND IPP.RequestId = @RequestId AND CAST(ppc.PerformedFromDate AS DATE)=CAST(IPP.DatePerformed AS DATE) AND IPP.DenomExceptions =1
	WHERE dg.dc_id=@DoctorCompanyId AND ppc.DoctorId = @DoctorId AND ppc.PerformedFromDate BETWEEN @StartDate AND @EndDate
	AND ppc.CODE='428191000124101' AND ppc.Status='Not Done'
	AND IPP.DenomExceptions IS NULL OR IPP.DenomExceptions = 0 
