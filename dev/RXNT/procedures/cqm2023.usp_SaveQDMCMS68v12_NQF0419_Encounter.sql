...
-- Description:	To populate QDM tables for the measure CMS68v12_NQF0419_Encounters
-- =============================================
CREATE    PROCEDURE [cqm2023].[usp_SaveQDMCMS68v12_NQF0419_Encounter]
	@RequestId BIGINT
AS
BEGIN
	...
	INNER JOIN cqm2023.SysLookupCMS68v12_NQF0419 cqm  WITH(NOLOCK) ON cqm.Code = proce.code AND proce.type='Encounter'
	LEFT JOIN cqm2023.PatientEncounterCodes pec WITH(NOLOCK) ON  pec.Code = proce.code AND pec.PerformedFromDate = proce.date_performed
	...
	INNER JOIN cqm2023.SysLookupCMS68v12_NQF0419 cqm  WITH(NOLOCK) ON cqm.Code = @DefaultEncCode
	...
	LEFT JOIN cqm2023.PatientEncounterCodes pec WITH(NOLOCK) ON pec.EncounterId = enc.enc_id AND pec.Code = @DefaultEncCode AND pec.PerformedFromDate = enc.enc_date
