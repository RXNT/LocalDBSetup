...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [prv].[SearchPrescribingDoctors]
	 @DoctorGroupId INT
AS
BEGIN
	SET NOCOUNT ON;
	...
	AND D.DR_ENABLED = 1  and loginlock = 0 and lowusage_lock = 0 AND (D.billing_enabled=0 or (D.billing_enabled=1 and D.billingDate > getdate()))  AND D.PRESCRIBING_AUTHORITY >= 3 order by dr_first_name,DR_LAST_NAME
END
GO
